<template>
  <div class="orderMenucontainer q-pa-lg-lg">
    <q-dialog v-model="showPersonDialog" maximized>
      <div class="q-pa-xs bg-white">
        ...
        <div class="row w-100 items-center">
          <div class="col-12 row items-center">
            <q-input
              ...
              <template v-slot:append>
                <q-icon name="event" class="cursor-pointer q-mr-xs">
                  <q-popup-proxy cover transition-show="scale" transition-hide="scale" v-model="showDatePicker2">
                    <q-date v-model="personForm.checkDate" @input="$emit('close')" color="accent" mask="YYYY-MM-DD">
            ...
            <q-input
              ...
              :loading="loading"
              label="訂購人名字" />
            <q-option-group :options="optionsGender" type="radio" v-model="personForm.gender" dense class="optionGroup" />
            <q-btn color="accent" class="q-ml-md btndelivery">
              <div v-show="!personForm.getDate">送達日</div>
              {{ personForm.getDate }}
              <q-popup-proxy cover v-model="showDatePicker" transition-show="scale" transition-hide="scale" style="width: 800px">
                <q-date v-model="personForm.getDate" @input="$emit('close')" color="accent" mask="YYYY-MM-DD">
        ...
        <div class="row w-100 items-center q-mt-md">
          ...
          <q-checkbox label="同訂購人資料" v-model="personForm.sameOrderCustomer" class="q-mr-md" />
          <q-option-group :options="optionsTaxid" type="radio" dense class="q-mx-md" v-model="personForm.noCompilation" />
          <q-input
        ...
        <div class="row w-100 items-center q-mt-md">
          <div class="col-4 q-pa-xs row">
            <q-input
              ...
              standout="bg-pink-2 text-black"
              class="col q-ma-xs statusSelect" />
            <q-option-group :options="optionsGender" type="radio" v-model="personForm.gender2" dense class="optionGroup" />
            <q-input
              class="statusSelect2"
              standout="bg-pink-2 text-black"
          ...
          <div class="col-8 row items-center">
            ...
            <q-select
              ...
              <template v-if="personForm.address" v-slot:prepend>
                <q-icon name="cancel" @click.stop.prevent="personForm.address = ''" class="cursor-pointer" />
              </template>
            </q-select>
            <q-select
              class="col q-ma-xs addressSelect"
              ...
              <template v-if="personForm.address2" v-slot:prepend>
                <q-icon name="cancel" @click.stop.prevent="personForm.address2 = ''" class="cursor-pointer" />
              </template>
            </q-select>
            <q-input
              standout="bg-pink-2 text-black"
              ...
              <template v-if="personForm.road" v-slot:prepend>
                <q-icon name="cancel" @click.stop.prevent="personForm.road = ''" class="cursor-pointer" />
              </template>
              <template v-if="personForm.address" v-slot:append>
                <q-btn v-if="personForm.address" round dense flat icon="add" @click="opendialogRoad(personForm.address, personForm.address2, personForm.road)" />
              </template>
              <q-menu no-focus no-parent-event v-model="menuState">
                <div v-if="road.roadselect.length > 0" class="row w-100 items-start">
                  <q-item
                ...
                <q-spinner v-else color="primary" size="3rem" class="q-ma-xl" :thickness="10" />
              </q-menu>
            </q-input>
          </div>
        </div>
        ...
        <div class="row w-100 items-center q-mt-md">
          ...
          <div class="col-8 column">
            ...
            <div class="col-12 row justify-end items-end text-center">
              <q-input class="col" standout="bg-pink-2" v-model="addressForm.lane" label="" ref="inputRef" :input-style="{ fontSize: '20px' }" />
              <div class="text-h6 col">巷</div>
              <q-input class="col" standout="bg-pink-2" v-model="addressForm.alley" label="" ref="inputRef" :input-style="{ fontSize: '20px' }" />
              <div class="text-h6 col">弄</div>

              <q-input class="col" standout="bg-pink-2" v-model="addressForm.no" label="" ref="inputRef" :input-style="{ fontSize: '20px' }" />
              <div class="text-h6 col">號</div>

              <q-input class="col" standout="bg-pink-2" v-model="addressForm.floor" label="" ref="inputRef" :input-style="{ fontSize: '20px' }" />
              <div class="text-h6 col">樓</div>
              <div class="text-h6 col">之</div>
              <q-input class="col" standout="bg-pink-2" v-model="addressForm.at" label="" ref="inputRef" :input-style="{ fontSize: '20px' }" />

              <q-input class="col" standout="bg-pink-2" v-model="addressForm.room" label="" ref="inputRef" :input-style="{ fontSize: '20px' }" />
            ...
            <div class="col-12 row items-center q-mt-md q-gutter-x-md">
              ...
              <q-select
                ...
                map-options
                use-input />
              <q-input standout="bg-pink-2 text-black" v-model="personForm.fuck" label="控單表" lazy-rules class="col" />
              <!-- <q-select
                standout="bg-pink-2 text-black"
        ...
        <div class="row w-100 items-center q-mt-md">
          <div class="col-12 row items-center">
            <q-input
              ...
              @input="showDialog2"
              class="col q-mx-xs" />
            <q-input standout="bg-pink-2 text-black" class="col q-mx-xs" v-model="mealRemark" label="內部備註" @input="showDialog" />
          </div>
        </div>
        <div class="row w-100 justify-end btnSubmit">
          <SmallKeypad v-show="keypad" @keypadInput="handleKeypadInput" class="col-3" />
          <div class="col-1">
            <q-btn icon="keyboard" @click="keypad = !keypad" round size="md" />
          </div>

          <q-btn outline class="col-1 submitBtn q-mx-xs" label="退出表單" @click="showPersonDialog = false" to="/search" />
          <q-btn outline class="col-1 submitBtn q-mx-xs" @click="addOrderFirst()" label="保留單" />
          <q-btn outline class="col-1 submitBtn q-mx-xs" @click="onReset()" label="清除表格" />
          <q-btn outline class="col-1 submitBtn" color="accent" @click="memerSave" label="下一步" />
        </div>
    ...
    <div class="row w-100 justify-start items-center q-pa-md bg-dark">
      <div v-show="!meal.id" class="q-ma-none text-h5 col-1 text-white">建立訂單</div>
      <div v-show="meal.id" class="q-ma-none text-h5 col-1 text-white">修改訂單</div>
      <p v-show="meal.id" class="q-ma-none col-2 text-white">訂單編號: {{ meal.id }}</p>
      <div class="col"></div>
      <div class="col-8 tableMember items-center justify-end row">
        ...
        <p v-show="personForm.hometel">訂購人室內電話: {{ personForm.prefix }} {{ personForm.hometel }} #{{ personForm.hometel2 }}</p>
      </div>
      <div class="col-1">
        <q-btn v-show="!meal.id" color="warning" round size="16px" @click="addPerson" icon="edit" class="q-mx-md" text-color="black" />
      </div>
    </div>
    <div v-if="personForm.memberId || meal.id || meal.order.id" class="row menu text-left w-100 items-center q-mb-md" data-order-panel>
      <q-select filled v-model="personForm.orderStatu" label="訂單狀態" :options="orderStatus" emit-value map-options color="accent" class="q-mx-lg w-300px col-2" />
      <div class="col-1"></div>
      <h5>開始點餐</h5>
      <q-btn @click="openbangdong" text-color="black" color="warning" label="便當" class="col" />
      <q-btn color="warning" label="盒餐" text-color="black" @click="openlunchBox" class="col" />
      <q-btn color="warning" label="單點" text-color="black" @click="opensolo" class="col" />
      <q-btn color="warning" label="客製便當" @click="openCustomBangdong" class="col" text-color="black" />
      <q-btn color="warning" label="客製盒餐" @click="openCustomLunchBox" class="col" text-color="black" />
      ...
      <q-dialog v-model="showbangdong">
        <q-card class="column" id="dialogbtnBangdongCard">
          <div class="col-2 row justify-end h-40px q-my-md">
            <div class="col-1">
              <q-btn @click="showbangdong = false" round icon="close" color="dark" />
            </div>
          </div>
          <!-- 便當按鈕 -->
        ...
        <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdong1Main, index) in bangdong1Mains" :key="index" class="flex-auto q-ma-none q-pa-none">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center" style="height: 100%">
              <q-item v-for="(bangdong1Drink, index) in bangdong1Drinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdong1SideDish, index) in bangdong1SideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 w-100 q-pa-xs q-mx-lg q-mt-xl">
            <div class="row w-100 justify-between">
              <q-input :bg-color="inputColor" outlined v-model="bangdong1Comment" label="商品備註" class="col-8" />

              <q-btn color="dark" label="取消" size="25px" @click="Resetbangdong1" class="q-mx-xs btnSubmitCancel" />
              <q-btn color="primary" label="送出" type="submit" size="25px" class="q-mx-xs btnSubmitCancel" @click="submitBangdong1" />
            </div>
          </div>
        </q-card>
      </q-dialog>
      ...
      <q-dialog v-model="showbangdong2" maximized>
        <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdong2Drink, index) in bangdong2Drinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdong2BentoMain, index) in bangdong2BentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdong2SideDish, index) in bangdong2SideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row q-mt-xl w-100 items-center justify-between">
              <q-input outlined :bg-color="inputColor" v-model="bangdong2Comment" label="商品備註" class="col-8" />
              <q-btn color="dark" label="取消" size="25px" class="q-mx-xs btnSubmitCancel" @click="Resetbangdong2" />
              <q-btn color="primary" label="送出" type="submit" size="25px" class="q-mx-xs btnSubmitCancel" @click="submitBangdong2" />
            </div>
          </div>
        </q-card>
      </q-dialog>
      ...
      <q-dialog v-model="showbangdong3" maximized>
        <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdong3Drink, index) in bangdong3Drinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdong3BentoMain, index) in bangdong3BentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdong3SideDish, index) in bangdong3SideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 w-100 q-pa-xs q-mx-lg q-mt-xl">
            <div class="row w-100 justify-between">
              <q-input :bg-color="inputColor" v-model="bangdong3Comment" label="商品備註" outlined class="col-8" />
              <q-btn color="dark" label="取消" @click="Resetbangdong3" class="q-mx-xs btnSubmitCancel" size="25px" />
              <q-btn color="primary" label="送出" type="submit" class="q-mx-xs btnSubmitCancel" size="25px" @click="submitBangdong3" />
            </div>
          </div>
        </q-card>
      </q-dialog>
      ...
      <q-dialog v-model="showbangdong4" maximized>
        <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdong4Drink, index) in bangdong4Drinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdong4BentoMain, index) in bangdong4BentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdong4SideDish, index) in bangdong4SideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="row items-center q-ml-md w-100 q-mt-xl">
            <q-input v-model="bangdong4Comment" label="商品備註" outlined :bg-color="inputColor" class="col-9" />
            <q-btn color="dark" label="取消" @click="Resetbangdong4" size="25px" class="q-mx-xs btnSubmitCancel" />
            <q-btn color="primary" label="送出" size="25px" class="q-mx-xs btnSubmitCancel" @click="submitBangdong4" />
          </div>
        </q-card>
      </q-dialog>
      <!-- 鮭魚 dialog -->
      <q-dialog v-model="showbangdongSalmon" maximized>
        <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongSalmonMain, index) in bangdongSalmonMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongSalmonDrink, index) in bangdongSalmonDrinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongSalmonBentoMain, index) in bangdongSalmonBentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongSalmonSideDish, index) in bangdongSalmonSideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="row items-center q-ml-md" style="width: 100%">
            <q-input v-model="bangdongSalmonComment" label="商品備註" :bg-color="inputColor" outlined class="col-9" />
            <q-btn color="dark" label="取消" size="25px" @click="ResetbangdongSalmon" class="q-mx-xs btnSubmitCancel" />
            <q-btn color="primary" label="送出" type="submit" size="25px" class="q-mx-xs btnSubmitCancel" @click="submitBangdongSalmon" />
          </div>
        </q-card>
      </q-dialog>
      <!-- 素食便當 125dialog -->
      <q-dialog v-model="showbangdongVegetarian1" maximized>
        <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongVegetarian1Main, index) in bangdongVegetarian1Mains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongVegetarian1Drink, index) in bangdongVegetarian1Drinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongVegetarian1BentoMain, index) in bangdongVegetarian1BentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
                  v-model.number="bangdongVegetarian1BentoMain.quantity"
                  :bg-color="getInputColor(bangdongVegetarian1BentoMain.quantity)"
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongVegetarian1SideDish, index) in bangdongVegetarian1SideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
                  v-model.number="bangdongVegetarian1SideDish.quantity"
                  :label="bangdongVegetarian1SideDish.name"
                  :bg-color="getInputColor(bangdongVegetarian1SideDish.quantity)"
                  filled
                  type="number"
                  :input-style="{ fontSize: '20px', margin: '0' }"
                  :label-style="{ fontSize: '20px' }"
          ...
          <div class="row items-center q-ml-md" style="width: 100%">
            <q-input v-model="bangdongVegetarian1Comment" label="商品備註" :bg-color="inputColor" class="col-9" outlined />
            <q-btn color="dark" label="取消" size="25px" @click="ResetbangdongVegetarian1" class="q-mx-xs btnSubmitCancel" />
            <q-btn color="primary" label="送出" type="submit" class="q-mx-xs btnSubmitCancel" size="25px" @click="submitBangdongVegetarian1" />
          </div>
        </q-card>
      </q-dialog>
      <!-- 素食便當 150dialog -->
      <q-dialog v-model="showbangdongVegetarian2" maximized>
        <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongVegetarian2Main, index) in bangdongVegetarian2Mains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongVegetarian2Drink, index) in bangdongVegetarian2Drinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongVegetarian2BentoMain, index) in bangdongVegetarian2BentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
                  v-model.number="bangdongVegetarian2BentoMain.quantity"
                  :label="bangdongVegetarian2BentoMain.name"
                  filled
                  :bg-color="getInputColor(bangdongVegetarian2BentoMain.quantity)"
                  type="number"
                  :input-style="{ fontSize: '20px', margin: '0' }"
                  :label-style="{ fontSize: '20px' }"
                  class="q-ma-xs w-110px"
          ...
          <div class="color1 bangdongdialog">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongVegetarian2SideDish, index) in bangdongVegetarian2SideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
                  v-model.number="bangdongVegetarian2SideDish.quantity"
                  :bg-color="getInputColor(bangdongVegetarian2SideDish.quantity)"
          ...
          <!-- 總計 -->
          <div class="row items-center q-ml-md" style="width: 100%">
            <q-input v-model="bangdongVegetarian2Comment" label="商品備註" :bg-color="inputColor" class="col-9" outlined />
            <q-btn color="dark" label="取消" size="25px" @click="ResetbangdongVegetarian2" class="q-mx-xs btnSubmitCancel" />
            <q-btn color="primary" label="送出" type="submit" class="q-mx-xs btnSubmitCancel" size="25px" @click="submitBangdongVegetarian2" />
          </div>
        </q-card>
      </q-dialog>
      <!-- 盒餐 dailog -->
      <q-dialog v-model="showlunchBox" class="lunchBoxdailog">
        <q-card class="column" id="dialogbtnLunchBoxCard">
          <div class="w-100 col row justify-end h-40px q-my-md">
            <div class="col-1">
              <q-btn @click="showlunchBox = false" round icon="close" color="dark" />
            </div>
          </div>
          <!-- 盒餐按鈕 -->
          <div class="items-center justify-center row w-100 q-ma-auto col">
            <q-btn
              color="dark"
      ...
      <q-dialog v-model="showlunchBox1" maximized>
        ...
        <q-card style="overflow-x: hidden">
          ...
          <div class="row items-center" style="width: 100%">
            <q-input v-model="lunchbox1Comment" label="商品備註" :bg-color="inputColor" class="q-ml-lg col-8" />

            <q-btn color="dark" label="取消" size="30px" @click="Resetlunchbox1" class="q-ma-lg col-1" />
            <q-btn color="primary" label="送出" type="submit" class="q-ma-lg col-1" size="30px" @click="submitLunchbox1" />
          </div>
        </q-card>
      </q-dialog>
      ...
      <q-dialog v-model="showlunchBox2" maximized>
        ...
        <q-card style="overflow-x: hidden">
          ...
          <div class="color1 lunchbox11">
            ...
            <div class="row items-center">
              <q-item v-for="(lunchBox2SideDish, index) in lunchBox2SideDishes" :key="index" class="flex-auto" :bg-color="inputColor" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="row items-center w-100 q-ml-md">
            <q-input v-model="lunchbox2Comment" label="商品備註" :bg-color="inputColor" class="col-9" />
            <q-btn color="dark" label="取消" size="30px" @click="Resetlunchbox2" class="q-ma-lg" />
            <q-btn color="primary" label="送出" type="submit" class="q-ma-lg" size="30px" @click="submitLunchbox2" />
          </div>
        </q-card>
      </q-dialog>
      <!-- 貴賓盒餐 dailog -->
      <q-dialog v-model="showlunchBox3" maximized>
        ...
        <q-card style="overflow-x: hidden">
          ...
          <div class="color1 lunchbox11">
            ...
            <div class="row items-center">
              <q-item v-for="(lunchBox3SideDish, index) in lunchBox3SideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <div class="row items-center q-ml-md">
            <q-input v-model="lunchbox3Comment" label="商品備註" :bg-color="inputColor" class="col-9" />
            <q-btn color="dark" label="取消" @click="Resetlunchbox3" class="q-ma-lg" size="30px" />
            <q-btn color="primary" label="送出" type="submit" class="q-ma-lg" @click="submitLunchbox3" size="30px" />
          </div>
        </q-card>
      </q-dialog>
      <!-- 首席盒餐 dailog -->
      <q-dialog v-model="showlunchBox4" maximized>
        ...
        <q-card style="overflow-x: hidden">
          ...
          <div class="color1 lunchbox11">
            ...
            <div class="row items-center">
              <q-item v-for="(lunchBox4SideDish, index) in lunchBox4SideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
          ...
          <!-- 按鈕 -->
          <div class="row items-center w-100 q-ml-md">
            <q-input v-model="lunchbox4Comment" label="商品備註" :bg-color="inputColor" class="col-9" />

            <q-btn color="dark" label="取消" size="30px" @click="Resetlunchbox4" class="q-ma-lg" />
            <q-btn color="primary" label="送出" type="submit" class="q-ma-lg" size="30px" @click="submitLunchbox4" />
          </div>
        </q-card>
      </q-dialog>
      ...
      <q-dialog v-model="showbangdongSolo" maximized>
        <q-card class="q-pa-md row items-start word-break">
          ...
          <div class="color1 w-100 q-pa-none q-ma-none">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongSoloDrink, index) in bangdongSoloDrinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
                  min="0"
          ...
          <div class="color1 w-100 q-pa-none q-ma-none">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongSoloBentoMain, index) in bangdongSoloBentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
                  min="0"
          ...
          <div class="color1 w-100 q-pa-none q-ma-none">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongSoloSideDish, index) in bangdongSoloSideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
                  min="0"
          ...
          <div class="color1 w-100 q-pa-none q-ma-none">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongSoloEcoBag, index) in bangdongSoloEcoBags" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
                  min="0"
          ...
          <div class="row w-100 items-center q-ml-md">
            <q-input v-model="bangdongSoloComment" label="商品備註" :bg-color="inputColor" class="col-9" />

            <q-btn color="dark" label="取消" @click="ResetbangdongSolo" class="q-ma-lg" size="30px" />
            <q-btn color="primary" label="送出" type="submit" class="q-ma-lg" size="30px" @click="submitBangdongSolo" />
          </div>
        </q-card>
      </q-dialog>
      ...
      <q-dialog v-model="showbangdongCustom1" class="lunchBoxdailog" maximized>
        <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
          ...
          <div class="color1 w-100 q-pa-none q-ma-none">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongCustom1Drink, index) in bangdongCustom1Drinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
                  min="0"
          ...
          <div class="color1 w-100 q-pa-none q-ma-none">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongCustom1BentoMain, index) in bangdongCustom1BentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
                  min="0"
          ...
          <div class="color1 w-100 q-pa-none q-ma-none">
            ...
            <div class="row items-center">
              <q-item v-for="(bangdongCustom1SideDish, index) in bangdongCustom1SideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
                  min="0"
          ...
          <!-- 按鈕 -->
          <div class="row w-100 items-center">
            <q-input v-model="bangdongCustom1Comment" label="商品備註" :bg-color="inputColor" class="col-9" />

            <q-btn color="dark" label="取消" @click="ResetbangdongCustom1" class="q-mx-lg" size="30px" />
            <q-btn color="primary" label="送出" type="submit" class="q-mx-lg" size="30px" @click="submitBangdongCustom1" />
          </div>
        </q-card>
      </q-dialog>
      ...
      <q-dialog v-model="showLunchBoxCustom1" class="lunchBoxdailog" maximized>
        ...
        <q-card class="q-pa-xs" style="overflow-x: hidden">
          ...
          <div class="color1 w-100 q-pa-none q-ma-none">
            ...
            <div class="row items-center">
              <q-item v-for="(lunchBoxCustom1Burrito6inch, index) in lunchBoxCustom1Burrito6inchs" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
                  min="0"
                  ...
                  filled
                  type="number"
                  :bg-color="getInputColor(lunchBoxCustom1Burrito6inch.quantity)"
                  class="q-ma-xs w-110px"
                  :input-style="{ fontSize: '20px', margin: '0' }"
                  :label-style="{ fontSize: '20px' }" />
              </q-item>
          ...
          <div class="color1 w-100 q-pa-none q-ma-none">
            ...
            <div class="row items-center">
              <q-item v-for="(lunchBoxCustom1SideDish, index) in lunchBoxCustom1SideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
                  min="0"
          ...
          <div class="color1 w-100 q-pa-none q-ma-none">
            ...
            <div class="row items-center">
              <q-item v-for="(lunchBoxCustom1BentoMain, index) in lunchBoxCustom1BentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                <q-input
                  min="0"
          ...
          <div class="row items-center w-100 q-mt-md">
            <q-input v-model="lunchboxCustom1Comment" label="商品備註" :bg-color="inputColor" class="col-9" />

            <q-btn color="dark" label="取消" @click="ResetLunchBoxCustom1" class="q-mx-lg" size="30px" />
            <q-btn color="primary" label="送出" type="submit" class="q-mx-lg" size="30px" @click="submitLunchboxCustom1" />
          </div>
        </q-card>
      </q-dialog>
    </div>
      ...
      <div class="col p-4 slider" data-order-lists>
                ...
                <template #body="props">
                  <q-tr :props="props">
                    ...
                    <q-td class="tdname">
                      ...
                      <div v-for="(input, index) in props.row.inputs" :key="index" class="tdname">
                    ...
                    <q-td class="tdmain">
                      {{ props.row.price }}
                      <div v-for="(productInput, index) in props.row.productInputs" :key="index" class="tdname">
                    ...
                    <q-td class="tdedit">
                      <div class="row w-100">
                        <div class="col">
                          <q-btn icon="delete" color="warning" text-color="black" size="lg" round @click="deleteRow(props.row.id)" />
                          <q-btn icon="edit" round size="lg" color="warning" text-color="black" class="q-ml-xs" @click="editRowDialog(props.row)" />
                        </div>
                      </div>
                    </q-td>
                    <q-td auto-width>
                      <q-toggle color="accent" v-model="props.expand" checked-icon="add" unchecked-icon="remove" />
                    </q-td>
                  </q-tr>
        ...
        <q-dialog v-model="editDialog1" maximized>
          <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
            ...
            <div class="color1 bangdongdialog">
              <h5 style="margin: 0; padding: 0">{{ bangdong1DrinkName }}</h5>
              <div class="row items-center">
                <q-item v-for="(bangdong1EditDrink, index) in bangdong1EditDrinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              <h5 style="margin: 0; padding: 0">{{ bangdong1SideDishName }}</h5>
              <div class="row items-center">
                <q-item v-for="(bangdong1EditSideDish, index) in bangdong1EditSideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="row items-center" style="width: 100%">
              <q-input v-model="bangdong1Comment" outlined label="商品備註" class="color1 col-9" :bg-color="inputColor" />
              <q-btn label="取消" color="dark" size="25px" @click="closeEditDialog1" class="q-mx-xs btnSubmitCancel" />
              <q-btn label="儲存" color="primary" size="25px" @click="saveEditDialog1" class="q-mx-xs btnSubmitCancel" />
            </div>
          </q-card>
        </q-dialog>
        ...
        <q-dialog v-model="editDialog2" maximized>
          <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
            ...
            <div class="color1 bangdongdialog">
              <h5 style="margin: 0; padding: 0">{{ bangdong2DrinkName }}</h5>
              <div class="row items-center">
                <q-item v-for="(bangdong2EditDrink, index) in bangdong2EditDrinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdong2EditBentoMain, index) in bangdong2EditBentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              <h5 style="margin: 0; padding: 0">{{ bangdong2SideDishName }}</h5>
              <div class="row items-center">
                <q-item v-for="(bangdong2EditSideDish, index) in bangdong2EditSideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <!-- 按鈕 -->
            <div class="row items-center" style="width: 100%">
              <q-input v-model="bangdong2Comment" outlined label="商品備註" class="col-8" :bg-color="inputColor" />
              <q-btn label="取消" color="dark" size="25px" class="q-mx-xs btnSubmitCancel" @click="closeEditDialog2" />
              <q-btn label="儲存" color="primary" size="25px" @click="saveEditDialog2" class="q-mx-xs btnSubmitCancel" />
            </div>
          </q-card>
        </q-dialog>
        ...
        <q-dialog v-model="editDialog3" maximized>
          <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
            ...
            <div class="color1 bangdongdialog">
              <h5 style="margin: 0; padding: 0">{{ bangdong3MainName }}</h5>
              <div class="row items-center">
                <q-item v-for="(bangdong3EditMain, index) in bangdong3EditMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              <h5 style="margin: 0; padding: 0">{{ bangdong3DrinkName }}</h5>
              <div class="row items-center">
                <q-item v-for="(bangdong3EditDrink, index) in bangdong3EditDrinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdong3EditBentoMain, index) in bangdong3EditBentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdong3EditSideDish, index) in bangdong3EditSideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              <!-- 按鈕 -->
              <div class="row items-center" style="width: 100%">
                <q-input :bg-color="inputColor" v-model="bangdong3Comment" outlined label="商品備註" class="col-8" />
                <q-btn color="dark" label="取消" @click="closeEditDialog3" size="25px" class="q-mx-xs btnSubmitCancel" />
                <q-btn color="primary" label="送出" type="submit" size="25px" class="q-mx-xs btnSubmitCancel" @click="saveEditDialog3" />
              </div>
            </div>
          </q-card>
        </q-dialog>
        ...
        <q-dialog v-model="editDialog4" maximized>
          <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
            ...
            <div class="color1 bangdongdialog">
              <h5 style="margin: 0; padding: 0">{{ bangdong4MainName }}</h5>
              <div class="row items-center">
                <q-item v-for="(bangdong4EditMain, index) in bangdong4EditMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              <h5 style="margin: 0; padding: 0">{{ bangdong4DrinkName }}</h5>
              <div class="row items-center">
                <q-item v-for="(bangdong4EditDrink, index) in bangdong4EditDrinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdong4EditBentoMain, index) in bangdong4EditBentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              <h5 style="margin: 0; padding: 0">{{ bangdong4SideDishName }}</h5>
              <div class="row items-center">
                <q-item v-for="(bangdong4EditSideDish, index) in bangdong4EditSideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              <!-- 按鈕 -->
              <div class="row items-center q-ml-md" style="width: 100%">
                <q-input v-model="bangdong4Comment" outlined label="商品備註" class="color1 col-8" :bg-color="inputColor" />
                <q-btn label="取消" color="dark" @click="closeEditDialog4" size="25px" class="q-mx-xs btnSubmitCancel" />
                <q-btn label="儲存" color="primary" @click="saveEditDialog4" size="25px" class="q-mx-xs btnSubmitCancel" />
              </div>
            </div>
          </q-card>
        </q-dialog>
        ...
        <q-dialog v-model="editDialogSalmon" maximized>
          <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongSalmonEditMain, index) in bangdongSalmonEditMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongSalmonEditDrink, index) in bangdongSalmonEditDrinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongSalmonEditBentoMain, index) in bangdongSalmonEditBentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
                    v-model.number="bangdongSalmonEditBentoMain.quantity"
                    :bg-color="getInputColor(bangdongSalmonEditBentoMain.quantity)"
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongSalmonEditSideDish, index) in bangdongSalmonEditSideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
                    v-model.number="bangdongSalmonEditSideDish.quantity"
                    :label="bangdongSalmonEditSideDish.name"
                    filled
                    :bg-color="getInputColor(bangdongSalmonEditSideDish.quantity)"
                    type="number"
                    :input-style="{ fontSize: '20px', margin: '0' }"
                    :label-style="{ fontSize: '20px' }"
                    class="q-pa-xs w-110px"
            ...
            <div class="color1 bangdongdialog">
              <!-- 按鈕 -->
              <div class="row items-center q-ml-md" style="width: 100%">
                <q-input v-model="bangdongSalmonComment" outlined label="商品備註" class="color1 col-8" :bg-color="inputColor" />
                <q-btn label="取消" color="dark" @click="closeEditDialogSalmon" size="25px" class="q-mx-xs btnSubmitCancel" />
                <q-btn label="儲存" color="primary" @click="saveEditDialogSalmon" size="25px" class="q-mx-xs btnSubmitCancel" />
              </div>
            </div>
          </q-card>
        </q-dialog>
        ...
        <q-dialog v-model="editDialogV1" maximized>
          <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
            <div class="color2 bangdongdialog row items-center text-white">
              ...
              <div class="text-h5 q-ml-md col-2 row items-center">
                ...
                <q-input
                  ...
                  label="數量總計"
                  :bg-color="getInputColor(bangdongVegetarian1EditTotalQuantity)"
                  min="0" />
              </div>
              <div class="col"></div>
              <q-input
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongVegetarian1EditMain, index) in bangdongVegetarian1EditMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
                    ...
                    class="q-pa-xs w-110px"
                    min="0"
                    :bg-color="getInputColor(bangdongVegetarian1EditMain.quantity)" />
                </q-item>
              </div>
            </div>
            <!-- 飲料 -->
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongVegetarian1EditDrink, index) in bangdongVegetarian1EditDrinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
                    v-model.number="bangdongVegetarian1EditDrink.quantity"
                    :bg-color="getInputColor(bangdongVegetarian1EditDrink.quantity)"
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item
                  v-for="(bangdongVegetarian1EditBentoMain, index) in bangdongVegetarian1EditBentoMains"
                  :key="index"
                  class="flex-auto"
                  style="margin: 0; padding: 0">
                  <q-input
                    ...
                    :bg-color="getInputColor(bangdongVegetarian1EditBentoMain.quantity)"
                    filled
                    type="number"
                    :input-style="{ fontSize: '20px', margin: '0' }"
                    :label-style="{ fontSize: '20px' }"
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item
                  v-for="(bangdongVegetarian1EditSideDish, index) in bangdongVegetarian1EditSideDishes"
                  :key="index"
                  class="flex-auto"
                  style="margin: 0; padding: 0">
                  <q-input
                    ...
                    :bg-color="getInputColor(bangdongVegetarian1EditSideDish.quantity)"
                    filled
                    type="number"
                    :input-style="{ fontSize: '20px', margin: '0' }"
                    :label-style="{ fontSize: '20px' }"
            ...
            <div class="color1 bangdongdialog">
              <!-- 按鈕 -->
              <div class="row items-center" style="width: 100%">
                <q-input v-model="bangdongVegetarian1Comment" outlined label="商品備註" class="color1 col-8" :bg-color="inputColor" />
                <q-btn label="取消" color="dark" @click="closeEditDialogV1" size="25px" class="q-mx-xs btnSubmitCancel" />
                <q-btn label="儲存" color="primary" @click="saveEditDialogV1" size="25px" class="q-mx-xs btnSubmitCancel" />
              </div>
            </div>
          </q-card>
        </q-dialog>
        ...
        <q-dialog v-model="editDialogV2" maximized>
          <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
            <div class="color2 bangdongdialog row items-center text-white">
              ...
              <div class="text-h5 q-ml-md col-2 row items-center">
                ...
                <q-input
                  ...
                  label="數量總計"
                  :bg-color="getInputColor(bangdongVegetarian2EditTotalQuantity)"
                  min="0" />
              </div>
              <div class="col"></div>
              <q-input
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongVegetarian2EditMain, index) in bangdongVegetarian2EditMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
                    v-model.number="bangdongVegetarian2EditMain.quantity"
                    :label="bangdongVegetarian2EditMain.shortname"
                    filled
                    type="number"
                    :bg-color="getInputColor(bangdongVegetarian2EditMain.quantity)"
                    :input-style="{ fontSize: '20px', margin: '0' }"
                    :label-style="{ fontSize: '20px' }"
                    class="q-pa-xs w-110px"
                    min="0" />
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongVegetarian2EditDrink, index) in bangdongVegetarian2EditDrinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
                    v-model.number="bangdongVegetarian2EditDrink.quantity"
                    :label="bangdongVegetarian2EditDrink.name"
                    filled
                    type="number"
                    :bg-color="getInputColor(bangdongVegetarian2EditDrink.quantity)"
                    :input-style="{ fontSize: '20px', margin: '0' }"
                    :label-style="{ fontSize: '20px' }"
                    class="q-pa-xs w-110px"
                    min="0" />
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item
                  v-for="(bangdongVegetarian2EditBentoMain, index) in bangdongVegetarian2EditBentoMains"
                  :key="index"
                  class="flex-auto"
                  style="margin: 0; padding: 0">
                  <q-input
                    ...
                    filled
                    type="number"
                    :bg-color="getInputColor(bangdongVegetarian2EditBentoMain.quantity)"
                    :input-style="{ fontSize: '20px', margin: '0' }"
                    :label-style="{ fontSize: '20px' }"
                    class="q-pa-xs w-110px"
                    min="0" />
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item
                  v-for="(bangdongVegetarian2EditSideDish, index) in bangdongVegetarian2EditSideDishes"
                  :key="index"
                  class="flex-auto"
                  style="margin: 0; padding: 0">
                  <q-input
                    ...
                    filled
                    type="number"
                    :bg-color="getInputColor(bangdongVegetarian2EditSideDish.quantity)"
                    :input-style="{ fontSize: '20px', margin: '0' }"
                    :label-style="{ fontSize: '20px' }"
                    class="q-pa-xs w-110px"
                    min="0" />
            ...
            <div class="color1 bangdongdialog">
              <!-- 按鈕 -->
              <div class="row items-center" style="width: 100%">
                <q-input v-model="bangdongVegetarian2Comment" outlined label="商品備註" class="color1 col-8" :bg-color="inputColor" />
                <q-btn label="取消" color="dark" @click="closeEditDialogV2" size="25px" class="q-mx-xs btnSubmitCancel" />
                <q-btn label="儲存" color="primary" @click="saveEditDialogV2" size="25px" class="q-mx-xs btnSubmitCancel" />
              </div>
            </div>
          </q-card>
        </q-dialog>
        ...
        <q-dialog v-model="editDialog9" maximized>
          <q-card class="items-start word-break q-pa-none q-ma-none" id="bangdongcard">
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongCustom1EditMain, index) in bangdongCustom1EditMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongCustom1EditDrink, index) in bangdongCustom1EditDrinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongCustom1EditBentoMain, index) in bangdongCustom1EditBentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
                    v-model.number="bangdongCustom1EditBentoMain.quantity"
                    :label="bangdongCustom1EditBentoMain.name"
                    filled
                    type="number"
                    :bg-color="getInputColor(bangdongCustom1EditBentoMain.quantity)"
                    :input-style="{ fontSize: '20px', margin: '0' }"
                    :label-style="{ fontSize: '20px' }"
                    class="q-pa-xs w-110px"
                    min="0" />
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongCustom1EditSideDish, index) in bangdongCustom1EditSideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
                    v-model.number="bangdongCustom1EditSideDish.quantity"
                    :label="bangdongCustom1EditSideDish.name"
                    filled
                    type="number"
                    :bg-color="getInputColor(bangdongCustom1EditSideDish.quantity)"
                    :input-style="{ fontSize: '20px', margin: '0' }"
                    :label-style="{ fontSize: '20px' }"
                    class="q-pa-xs w-110px"
                    min="0" />
            ...
            <div class="color1 bangdongdialog">
              <!-- 按鈕 -->
              <div class="row items-center" style="width: 100%">
                <q-input v-model="bangdongCustom1Comment" outlined label="商品備註" class="color1 col-8" :bg-color="inputColor" />
                <q-btn label="取消" color="dark" @click="closeEditDialog9" size="25px" class="q-mx-xs btnSubmitCancel" />
                <q-btn label="儲存" color="primary" @click="saveEditDialog9" size="25px" class="q-mx-xs btnSubmitCancel" />
              </div>
            </div>
          </q-card>
        </q-dialog>
        ...
        <q-dialog v-model="editDialog5" maximized>
          <q-card>
            <div class="row items-start justify-center" id="lunchbox1">
              <div class="color2 bangdongdialog row items-center text-white">
                <h5 style="margin: 0; padding: 0">{{ lunchBox1EditName }} 總計</h5>
                <q-input
                  filled
                  type="number"
                  :input-style="{ fontSize: '20px' }"
            ...
            <div class="row items-center" style="width: 100%">
              <q-input v-model="lunchbox1Comment" outlined label="商品備註" class="color1 col-9 q-mx-lg" :bg-color="inputColor" />
              <q-btn color="dark" label="取消" @click="closeEditDialog5" size="30px" class="q-ma-xs col-1" />
              <q-btn color="primary" label="儲存" type="submit" size="30px" class="q-ma-xs col-1" @click="saveEditDialog5" />
            </div>
          </q-card>
        </q-dialog>
        <!-- 會議盒餐 -->
        <q-dialog v-model="editDialog6" maximized>
          <q-card>
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(lunchBox2EditSideDish, index) in lunchBox2EditSideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              <!-- 按鈕 -->
              <div class="row items-center" style="width: 100%">
                <q-input v-model="lunchbox2Comment" outlined label="商品備註" class="color1 col-9 q-mx-lg" :bg-color="inputColor" />

                <q-btn color="dark" label="取消" @click="closeEditDialog6" size="30px" class="q-ma-xs" />
                <q-btn color="primary" label="儲存" type="submit" size="30px" class="q-ma-xs" @click="saveEditDialog6" />
              </div>
            </div>
          </q-card>
        </q-dialog>
        ...
        <q-dialog v-model="editDialog7" maximized>
          <q-card>
            <div class="row items-start justify-center q-gutter-xs" id="lunchbox3Edit">
              <div class="color2 bangdongdialog row items-center text-white">
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(lunchBox3EditSideDish, index) in lunchBox3EditSideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              <!-- 按鈕 -->
              <div class="row items-center" style="width: 100%">
                <q-input v-model="lunchbox3Comment" outlined label="商品備註" class="color1 col-9 q-mx-lg" :bg-color="inputColor" />

                <q-btn color="dark" label="取消" @click="closeEditDialog7" size="30px" class="q-ma-xs col-1" />
                <q-btn color="primary" label="儲存" type="submit" size="30px" class="q-ma-xs col-1" @click="saveEditDialog7" />
              </div>
            </div>
          </q-card>
        </q-dialog>
        ...
        <q-dialog v-model="editDialog8" maximized>
          <q-card>
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(lunchBox4EditSideDish, index) in lunchBox4EditSideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
            ...
            <div class="color1 bangdongdialog">
              <!-- 按鈕 -->
              <div class="row items-center" style="width: 100%">
                <q-input v-model="lunchbox4Comment" outlined label="商品備註" class="color1 col-9 q-mx-lg" :bg-color="inputColor" />

                <q-btn color="dark" label="取消" @click="closeEditDialog8" size="30px" class="q-ma-xs col-1" />
                <q-btn color="primary" label="儲存" type="submit" size="30px" class="q-ma-xs col-1" @click="saveEditDialog8" />
              </div>
            </div>
          </q-card>
        </q-dialog>
        ...
        <q-dialog v-model="editDialog0" maximized>
          <q-card>
            <div class="row items-start justify-center" id="lunchBoxCustom1Edit">
              <div class="color2 bangdongdialog row items-center text-white">
                <h5 style="margin: 0; padding: 0">
            ...
            <div class="color1 dialogtitle">
              ...
              <div class="row items-center">
                <q-item v-for="(lunchBoxCustom1EditSideDish, index) in lunchBoxCustom1EditSideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
                    v-model.number="lunchBoxCustom1EditSideDish.quantity"
                    :label="lunchBoxCustom1EditSideDish.name"
                    filled
                    type="number"
                    :bg-color="getInputColor(lunchBoxCustom1EditSideDish.quantity)"
                    :input-style="{ fontSize: '20px', margin: '0' }"
                    :label-style="{ fontSize: '20px' }"
                    class="q-pa-xs w-110px"
                    min="0" />
            ...
            <div class="color1 w-100 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(lunchBoxCustom1EditBentoMain, index) in lunchBoxCustom1EditBentoMains" :key="index" class="flex-start" style="margin: 0; padding: 0">
                  <q-input
                    min="0"
                    ...
                    filled
                    type="number"
                    :bg-color="getInputColor(lunchBoxCustom1EditBentoMain.quantity)"
                    class="q-ma-xs w-110px"
                    :input-style="{ fontSize: '20px', margin: '0' }"
                    :label-style="{ fontSize: '20px' }" />
                </q-item>
            ...
            <div class="color1 bangdongdialog">
              <!-- 按鈕 -->
              <div class="row items-center" style="width: 100%">
                <q-input v-model="lunchboxCustom1Comment" outlined label="商品備註" class="color1 col-9 q-mx-lg" :bg-color="inputColor" />

                <q-btn color="dark" label="取消" @click="closeEditDialog0" size="30px" class="q-ma-xs col-1" />
                <q-btn color="primary" label="儲存" type="submit" size="30px" class="q-ma-xs col-1" @click="saveEditDialog0" />
              </div>
            </div>
          </q-card>
        </q-dialog>
        ...
        <q-dialog v-model="editDialog10" maximized>
          <q-card class="q-gutter-md row items-start word-break">
            ...
            <div class="color1 bangdongdialog">
              <h5 style="margin: 0; padding: 0">{{ bangdongSoloDrinkName }}</h5>
              <div class="row items-center">
                <q-item v-for="(bangdongSoloEditDrink, index) in bangdongSoloEditDrinks" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
                    min="0"
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongSoloEditBentoMain, index) in bangdongSoloEditBentoMains" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
                    min="0"
                    v-model.number="bangdongSoloEditBentoMain.quantity"
                    :bg-color="getInputColor(bangdongSoloEditBentoMain.quantity)"
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongSoloEditSideDish, index) in bangdongSoloEditSideDishes" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
                    min="0"
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-center">
                <q-item v-for="(bangdongSoloEditEcoBag, index) in bangdongSoloEditEcoBags" :key="index" class="flex-auto" style="margin: 0; padding: 0">
                  <q-input
                    min="0"
            ...
            <div class="color1 bangdongdialog">
              ...
              <div class="row items-cetner" style="width: 100%">
                <q-input v-model="bangdongSoloComment" outlined label="商品備註" class="color1 col-9 q-mx-lg" :bg-color="inputColor" />

                <q-btn color="dark" label="取消" @click="closeEditDialog10" size="30px" class="q-ma-xs col-1" />
                <q-btn color="primary" label="儲存" type="submit" size="30px" class="q-ma-xs col-1" @click="saveEditDialog10" />
              </div>
            </div>
          </q-card>
        </q-dialog>
        ...
        <div class="row q-mt-md border1px justify-center w-100">
          <div class="col-1 bg-dark text-center row items-center w-100 justify-center">
          ...
          <div class="col-5 items-center row">
            <div class="q-pa-xs row col justify-around items-center">
              ...
              <!-- <q-radio
                ...
                class="radio-option"
              /> -->
              <q-radio v-model="paymentMethod" val="debt" label="記帳" class="radio-option" />
              <q-radio v-model="paymentMethod" val="uber" label="uber" class="radio-option" />
              <q-input
                v-if="paymentMethod === 'debt' || paymentMethod === 'wire' || paymentMethod === 'credit'"
                ...
                <template v-slot:append>
                  <q-icon name="calendar_today" class="cursor-pointer" style="width: 10px">
                    <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                      <q-date v-model="getMoneyDate" today-btn color="accent" style="width: 800px" mask="YYYY-MM-DD">
                        <div class="row items-center justify-end">
                          <q-btn label="取消" color="dark" v-close-popup />
          ...
          <div class="col-2 text-center">
            <q-input filled v-model.number="discount" style="width: 90px" min="0" />
          </div>
          <div class="col-1 bg-dark text-center row items-center w-100 justify-center">
            <div class="text-h6 text-white">運費</div>
          </div>
          <div class="col-2 text-center">
            <q-input filled v-model.number="freight" style="width: 90px" min="0" />
          </div>
        </div>
        <div class="row border1px justify-center w-100">
          ...
          <div class="col-9 row q-px-lg">
            <div class="q-field row no-wrap items-center">
              ...
              <q-input filled v-model.number="total" style="width: 90px" min="0" />
            </div>
            <div class="q-field row no-wrap items-center">
            ...
            <div class="q-field row no-wrap items-center">
              <div class="col-auto text-accent" style="font-size: 20px">未付金額</div>
              <q-input filled v-model.number="unpaid" style="width: 90px" input-class="text-accent" />
            </div>
          </div>
          <div class="col-2 row justify-end"></div>
        </div>
        <div class="row justify-end w-100 q-mt-md">
          <q-btn outline v-if="!meal.id" color="accent" @click="alreadyCheck" :disabled="isSubmitting" label="完成訂單填寫，加入訂單" class="col-7 h-100px" />
          <q-btn outline v-if="meal.id" color="accent" :disabled="isSubmitting" label="完成修改訂單，儲存並覆蓋訂單" @click="alreadyCheck" class="col-7 h-100px" />
        </div>
      </div>
    </div>
  </div>
  ...
  <q-dialog v-model="mealDialog">
    <q-card>
      <q-card-section>
        <q-item v-for="Meal in optionsMeal" :key="Meal" clickable @click="selectOption(Meal)">
          <q-item-section>
            <q-item-label>{{ Meal }}</q-item-label>
          </q-item-section>
        </q-item>
        <q-pagination v-if="totalPagesMeal > 0" v-model="currentPagemeal" :min="1" :max="totalPagesMeal" :input="true" />
      </q-card-section>
  ...
  <q-dialog v-model="dialogVisible2" persistent>
    <q-card>
      <q-card-section>
        <q-item v-for="option2 in option2s" :key="option2" clickable @click="selectOption2(option2)">
          <q-item-section>
  ...
  <q-dialog v-model="dialogRoad" persistent full-width>
    <q-card>
      <q-card-section>
        <div class="row w-100 items-center">
          <q-input v-model="searchTerm" placeholder="搜尋..." dense class="q-mb-md col-10" />
          <div class="col-2">
            <q-btn label="取消" class="q-pl-md" color="dark" @click="dialogRoad = false" />
          </div>
        </div>
        <div class="row w-100 items-center" style="max-height: 100vh">
          <q-btn
            ...
            :key="road.id"
            clickable
            @click="UseRoadStore().getRoadFull(personForm.address, personForm.address2, road.value)">
            {{ road.value }}
          </q-btn>
          <div class="row w-100 items-baseline">
            <q-btn v-for="road in road.roadsFull" :key="road.id" clickable class="fwordbtn q-ma-xs" color="primary" @click="copyroad(road.value)">
              {{ road.value }}
            </q-btn>
          </div>
        </div>
        <q-pagination v-if="totalPages > 0" v-model="currentPage" :min="1" :max="totalPages" :input="true" />
      </q-card-section>
    </q-card>
  </q-dialog>
  <!-- 確認 -->
  <q-dialog v-model="confirm" persistent>
    <q-card>
      ...
      <div class="row w-100 justify-center">
        <q-btn label="再檢查" color="dark" v-close-popup size="20px" class="col q-ma-md" />
        <q-btn v-if="!meal.id" label="我確定" color="accent" size="20px" class="col q-ma-md" v-close-popup @click="addOrder()" />
        <q-btn v-if="meal.id" label="我確定喔" color="accent" size="20px" class="col q-ma-md" v-close-popup @click="editOrder" />
      </div>
    </q-card>
  </q-dialog>
</template>
...
const handleKeypadInput = input => {
  ...
  // 處理非數字和特殊字符
  if (typeof input === 'string' && isNaN(Number(input)) && !['backspace', '*', ',', '+', ':', '-'].includes(input)) {
    return
  }
  if (activeObject.value && activeKey.value) {
    ...
    currentVal = currentVal !== undefined && currentVal !== null ? String(currentVal) : ''
...
watch(
  () => personForm.getDate,
  async (newVal, oldVal) => {
    const date = DateTime.fromFormat(newVal, 'yyyy-MM-dd').setZone('Asia/Taipei')
...
const justcopy = async () => {
  ...
  personForm.compilation = members.persontin
  let stateObject = address.state.find(item => item.value === members.personstateid)
  if (stateObject === undefined) {
    stateObject = address.state.find(item => item.label === members.personstateid)
  }
...
const getFindus = async () => {
  const { data } = await apiAuth.get('/common/term?filter_taxonomy_code=member_how_to_find_us')
  const findus = await data.data.filter(res => res.taxonomy_code === 'member_how_to_find_us')
  console.log(findus)
...
async function fetchRoadData() {
  try {
    ...
    if (containsChinese) {
      await road.getRoadSelectFilter(personForm.address, personForm.address2, personForm.road)
    }
...
watch(
  ...
  ([newCompany, newSameOrderCustomer, newName], [prevCompany, prevSameOrderCustomer, prevName]) => {
      ...
      timeoutId = setTimeout(async () => {
        try {
          ...
          if (data.data.length > 0 && data.data.length < 2) {
            ...
            // 導入舊資料縣市
            const stateObject = await apiAuth.get('localization/division/state').then(response => response.data.find(item => item.id === data.data[0].shipping_state_id))

            personForm.address = stateObject.name
            // 導入舊資料區域
            const cities = await apiAuth
              .get(`localization/division/city?equal_parent_id=${stateObject.id}`)
              .then(response => response.data.find(item => item.city_id === data.data[0].shipping_city_id))

            personForm.address2 = cities.name
            const addressParts = await address.parseAddress(data.data[0].shipping_address1)
...
const getExtraComments2 = async () => {
  try {
    ...
    // 延遲指定的時間
    await new Promise(resolve => setTimeout(resolve, delay))
    const response = await apiAuth.get('/sale/order/phrases/phrase_order_comment', {
      params: {
...
const getExtraComments = async () => {
  try {
    ...
    // 延遲指定的時間
    await new Promise(resolve => setTimeout(resolve, delay))
    const response = await apiAuth.get('/sale/order/phrases/phrase_order_extra_comment', {
      params: {
...
const memerSave = async () => {
  ...
  try {
    ...
    personForm.address2 = city !== undefined && typeof city.value !== 'undefined' ? city.value : ''
...
watch(bangdong1Mains, newMains => {
  ...
  bangdong1Arbitrarily.value = Math.max(0, parseInt(bangdong1TotalQuantity.value || 0) - mainQuantityTotal)
})
...
const bangdong1finalTotal = computed(() => {
  const totalPrice = bangdong1optionPrice.value + bangdong1TotalQuantityPrice.value
  return totalPrice
})
// 將資料推進 tableRows
const submitBangdong1 = () => {
  ...
  // 檢查總數量是否匹配
  const calculatedTotalQuantity = bangdong1Mains.reduce((acc, main) => acc + (main.quantity || 0), 0)
  ...
  for (const bangdong1Main of bangdong1Mains) {
    if (bangdong1Main.quantity > 0) {
      ...
      row.Main += bangdong1Main.name + ' x ' + parseInt(bangdong1Main.quantity).toString() + '<br>'
  ...
  for (const bangdong1Drink of bangdong1Drinks) {
    if (bangdong1Drink.quantity > 0) {
      row.drinks += bangdong1Drink.name + '  x  ' + parseInt(bangdong1Drink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
  ...
  for (const bangdong1SideDish of bangdong1SideDishes) {
    if (bangdong1SideDish.quantity > 0) {
      ...
      row.sideDishes += bangdong1SideDish.name + 'x' + parseInt(bangdong1SideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、配菜 ID
      const productInput = {
...
watch(bangdong2Mains, newMains => {
  ...
  bangdong2Arbitrarily.value = Math.max(0, parseInt(bangdong2TotalQuantity.value) - mainQuantityTotal)
})
...
const bangdong2finalTotal = computed(() => {
  const totalPrice = bangdong2optionPrice.value + bangdong2TotalQuantityPrice.value
  return totalPrice
})
// 將資料推進 tableRows
const submitBangdong2 = () => {
  ...
  // 檢查總數量是否匹配
  const calculatedTotalQuantity = bangdong2Mains.reduce((acc, main) => acc + (main.quantity || 0), 0)
  ...
  for (const bangdong2Main of bangdong2Mains) {
    if (bangdong2Main.quantity > 0) {
      row.Main += bangdong2Main.name + '  x  ' + parseInt(bangdong2Main.quantity).toString() + '<br>'
      // 保留 主餐名稱、主餐 ID
      const productInput = {
  ...
  for (const bangdong2Drink of bangdong2Drinks) {
    if (bangdong2Drink.quantity > 0) {
      row.drinks += bangdong2Drink.name + 'x' + parseInt(bangdong2Drink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
  ...
  for (const bangdong2SideDish of bangdong2SideDishes) {
    if (bangdong2SideDish.quantity > 0) {
      row.sideDishes += bangdong2SideDish.name + 'x' + parseInt(bangdong2SideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、配菜 ID
      const productInput = {
  ...
  for (const bangdong2BentoMain of bangdong2BentoMains) {
    if (bangdong2BentoMain.quantity > 0) {
      row.bentoMains += bangdong2BentoMain.name + ' x ' + parseInt(bangdong2BentoMain.quantity).toString() + '<br>'
      // 保留副主餐名稱、副主餐 ID
      const productInput = {
...
watch(bangdong3Mains, newMains => {
  ...
  bangdong3Arbitrarily.value = Math.max(0, parseInt(bangdong3TotalQuantity.value) - mainQuantityTotal)
})
...
const bangdong3finalTotal = computed(() => {
  const totalPrice = bangdong3optionPrice.value + bangdong3TotalQuantityPrice.value
  return totalPrice
})
// 將資料推進 tableRows
const submitBangdong3 = () => {
  ...
  // 檢查總數量是否匹配
  const calculatedTotalQuantity = bangdong3Mains.reduce((acc, main) => acc + (main.quantity || 0), 0)
  ...
  for (const bangdong3Main of bangdong3Mains) {
    if (bangdong3Main.quantity > 0) {
      row.Main += bangdong3Main.name + '  x  ' + parseInt(bangdong3Main.quantity).toString() + '<br>'
      // 保留 主餐名稱、主餐 ID
      const productInput = {
  ...
  for (const bangdong3Drink of bangdong3Drinks) {
    if (bangdong3Drink.quantity > 0) {
      row.drinks += bangdong3Drink.name + 'x' + parseInt(bangdong3Drink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
  ...
  for (const bangdong3SideDish of bangdong3SideDishes) {
    if (bangdong3SideDish.quantity > 0) {
      row.sideDishes += bangdong3SideDish.name + '  x  ' + parseInt(bangdong3SideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、配菜 ID
      const productInput = {
  ...
  for (const bangdong3BentoMain of bangdong3BentoMains) {
    if (bangdong3BentoMain.quantity > 0) {
      row.bentoMains += bangdong3BentoMain.name + 'x' + parseInt(bangdong3BentoMain.quantity).toString() + '<br>'
      // 保留副主餐名稱、副主餐 ID
      const productInput = {
...
watch(bangdong4Mains, newMains => {
  ...
  bangdong4Arbitrarily.value = Math.max(0, parseInt(bangdong4TotalQuantity.value) - mainQuantityTotal)
})
...
const bangdong4finalTotal = computed(() => {
  const totalPrice = bangdong4optionPrice.value + bangdong4TotalQuantityPrice.value
  return totalPrice
})
// 將資料推進 tableRows
const submitBangdong4 = () => {
  ...
  // 檢查總數量是否匹配
  const calculatedTotalQuantity = bangdong4Mains.reduce((acc, main) => acc + (main.quantity || 0), 0)
  ...
  for (const bangdong4Main of bangdong4Mains) {
    if (bangdong4Main.quantity > 0) {
      ...
      row.Main += bangdong4Main.name + 'x' + parseInt(bangdong4Main.quantity).toString() + '<br>'
      const productInput = {
  ...
  for (const bangdong4Drink of bangdong4Drinks) {
    if (bangdong4Drink.quantity > 0) {
      row.drinks += bangdong4Drink.name + 'x' + parseInt(bangdong4Drink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
  ...
  for (const bangdong4SideDish of bangdong4SideDishes) {
    if (bangdong4SideDish.quantity > 0) {
      row.sideDishes += bangdong4SideDish.name + 'x' + parseInt(bangdong4SideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、配菜 ID
      const productInput = {
  ...
  for (const bangdong4BentoMain of bangdong4BentoMains) {
    if (bangdong4BentoMain.quantity > 0) {
      row.bentoMains += bangdong4BentoMain.name + 'x' + parseInt(bangdong4BentoMain.quantity).toString() + '<br>'
      // 保留副主餐名稱、副主餐 ID
      const productInput = {
...
watch(bangdongSalmonMains, newMains => {
  ...
  bangdongSalmonArbitrarily.value = Math.max(0, parseInt(bangdongSalmonTotalQuantity.value) - mainQuantityTotal)
})
const bangdongSalmonTotalQuantityPrice = computed(() => {
  const totalPrice = bangdongSalmonTotalQuantity.value * bangdongSalmonPrice.value
...
const bangdongSalmonfinalTotal = computed(() => {
  const totalPrice = bangdongSalmonoptionPrice.value + bangdongSalmonTotalQuantityPrice.value
  return totalPrice
})
// 將資料推進 tableRows
const submitBangdongSalmon = () => {
  ...
  // 檢查總數量是否匹配
  const calculatedTotalQuantity = bangdongSalmonMains.reduce((acc, main) => acc + (main.quantity || 0), 0)
  console.log(calculatedTotalQuantity, bangdongSalmonTotalQuantity.value)
  if (calculatedTotalQuantity !== parseInt(bangdongSalmonTotalQuantity.value, 10)) {
    alert('主餐總數量與輸入的總數量不搭！')
    return
  }
  const nextId = tableRows.length > 0 ? tableRows[tableRows.length - 1].id + 1 : 1
  ...
  for (const bangdongSalmonMain of bangdongSalmonMains) {
    if (bangdongSalmonMain.quantity > 0) {
      row.Main += bangdongSalmonMain.name + 'x' + parseInt(bangdongSalmonMain.quantity).toString() + '<br>'
      const productInput = {
  ...
  for (const bangdongSalmonDrink of bangdongSalmonDrinks) {
    if (bangdongSalmonDrink.quantity > 0) {
      row.drinks += bangdongSalmonDrink.name + 'x' + parseInt(bangdongSalmonDrink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
  ...
  for (const bangdongSalmonSideDish of bangdongSalmonSideDishes) {
    if (bangdongSalmonSideDish.quantity > 0) {
      row.sideDishes += bangdongSalmonSideDish.name + 'x' + parseInt(bangdongSalmonSideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、配菜 ID
      const productInput = {
  ...
  for (const bangdongSalmonBentoMain of bangdongSalmonBentoMains) {
    if (bangdongSalmonBentoMain.quantity > 0) {
      row.bentoMains += bangdongSalmonBentoMain.name + 'x' + parseInt(bangdongSalmonBentoMain.quantity).toString() + '<br>'
      // 保留副主餐名稱、副主餐 ID
      const productInput = {
  ...
  if (bangdongSalmonArbitrarily.value > 0) {
    row.Main += '任意搭配' + 'x' + parseInt(bangdongSalmonArbitrarily.value).toString()
    const input = {
...
watch(bangdongVegetarian1Mains, newMains => {
  ...
  bangdongVegetarian1Arbitrarily.value = Math.max(0, parseInt(bangdongVegetarian1TotalQuantity.value) - mainQuantityTotal)
})

// 計算總價格
const bangdongVegetarian1TotalQuantityPrice = computed(() => {
  const totalPrice = bangdongVegetarian1TotalQuantity.value * bangdongVegetarian1Price.value
...
const bangdongVegetarian1finalTotal = computed(() => {
  const totalPrice = bangdongVegetarian1optionPrice.value + bangdongVegetarian1TotalQuantityPrice.value
  return totalPrice
})
// 將資料推進 tableRows
const submitBangdongVegetarian1 = () => {
  ...
  // 檢查總數量是否匹配
  const calculatedTotalQuantity = bangdongVegetarian1Mains.reduce((acc, main) => acc + (main.quantity || 0), 0)
  console.log(calculatedTotalQuantity, bangdongVegetarian1TotalQuantity.value)
  if (calculatedTotalQuantity !== parseInt(bangdongVegetarian1TotalQuantity.value, 10)) {
    alert('主餐總數量與輸入的總數量不搭！')
    return
  }
  const nextId = tableRows.length > 0 ? tableRows[tableRows.length - 1].id + 1 : 1
  ...
  for (const bangdongVegetarian1Main of bangdongVegetarian1Mains) {
    if (bangdongVegetarian1Main.quantity > 0) {
      row.Main += bangdongVegetarian1Main.name + 'x' + parseInt(bangdongVegetarian1Main.quantity) + '<br>'
      // 保留 主餐名稱、主餐 ID
      const productInput = {
  ...
  for (const bangdongVegetarian1Drink of bangdongVegetarian1Drinks) {
    if (bangdongVegetarian1Drink.quantity > 0) {
      row.drinks += bangdongVegetarian1Drink.name + 'x' + parseInt(bangdongVegetarian1Drink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
  ...
  for (const bangdongVegetarian1SideDish of bangdongVegetarian1SideDishes) {
    if (bangdongVegetarian1SideDish.quantity > 0) {
      row.sideDishes += bangdongVegetarian1SideDish.name + 'x' + parseInt(bangdongVegetarian1SideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、配菜 ID
      const productInput = {
  ...
  for (const bangdongVegetarian1BentoMain of bangdongVegetarian1BentoMains) {
    if (bangdongVegetarian1BentoMain.quantity > 0) {
      row.bentoMains += bangdongVegetarian1BentoMain.name + 'x' + parseInt(bangdongVegetarian1BentoMain.quantity).toString() + '<br>'
      // 保留副主餐名稱、副主餐 ID
      const productInput = {
...
watch(bangdongVegetarian2Mains, newMains => {
  ...
  bangdongVegetarian2Arbitrarily.value = Math.max(0, parseInt(bangdongVegetarian2TotalQuantity.value) - mainQuantityTotal)
})
// 計算總價格
const bangdongVegetarian2TotalQuantityPrice = computed(() => {
  const totalPrice = bangdongVegetarian2TotalQuantity.value * bangdongVegetarian2Price.value
  return totalPrice
})
...
const bangdongVegetarian2finalTotal = computed(() => {
  const totalPrice = bangdongVegetarian2optionPrice.value + bangdongVegetarian2TotalQuantityPrice.value
  return totalPrice
})
// 將資料推進 tableRows
const submitBangdongVegetarian2 = () => {
  ...
  // 檢查總數量是否匹配
  const calculatedTotalQuantity = bangdongVegetarian2Mains.reduce((acc, main) => acc + (main.quantity || 0), 0)
  console.log(calculatedTotalQuantity, bangdongVegetarian2TotalQuantity.value)
  if (calculatedTotalQuantity !== parseInt(bangdongVegetarian2TotalQuantity.value, 10)) {
    alert('主餐總數量與輸入的總數量不搭！')
    return
  }
  const nextId = tableRows.length > 0 ? tableRows[tableRows.length - 1].id + 1 : 1
  ...
  for (const bangdongVegetarian2Main of bangdongVegetarian2Mains) {
    if (bangdongVegetarian2Main.quantity > 0) {
      ...
      row.Main += bangdongVegetarian2Main.name + 'x' + parseInt(bangdongVegetarian2Main.quantity).toString() + '<br>'
      // 保留 主餐名稱、主餐 ID
      const productInput = {
  ...
  for (const bangdongVegetarian2Drink of bangdongVegetarian2Drinks) {
    if (bangdongVegetarian2Drink.quantity > 0) {
      row.drinks += bangdongVegetarian2Drink.name + 'x' + parseInt(bangdongVegetarian2Drink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
  ...
  for (const bangdongVegetarian2SideDish of bangdongVegetarian2SideDishes) {
    if (bangdongVegetarian2SideDish.quantity > 0) {
      row.sideDishes += bangdongVegetarian2SideDish.name + 'x' + parseInt(bangdongVegetarian2SideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、配菜 ID
      const productInput = {
  ...
  for (const bangdongVegetarian2BentoMain of bangdongVegetarian2BentoMains) {
    if (bangdongVegetarian2BentoMain.quantity > 0) {
      row.bentoMains += bangdongVegetarian2BentoMain.name + 'x' + parseInt(bangdongVegetarian2BentoMain.quantity).toString() + '<br>'
      // 保留副主餐名稱、副主餐 ID
      const productInput = {
...
watch(lunchBox1Mains, newMains => {
  ...
  lunchBox1Arbitrarily.value = Math.max(0, parseInt(lunchBox1TotalQuantity.value) - mainQuantityTotal)
})
...
const lunchbox1finalTotal = computed(() => {
  const totalPrice = lunchBox1TotalQuantityPrice.value + lunchBox1optionPrice.value
  return totalPrice
})
const submitLunchbox1 = () => {
  ...
  row.quantity = totalQuantity
  for (const option of lunchBox1Mains) {
    if (option.quantity > 0 && option.name !== '主廚潤餅 3 吋') {
...
watch(lunchBox2Mains, newMains => {
  ...
  lunchBox2Arbitrarily.value = Math.max(0, parseInt(lunchBox2TotalQuantity.value) - mainQuantityTotal)
})
...
const lunchbox2finalTotal = computed(() => {
  const totalPrice = lunchBox2TotalQuantityPrice.value + lunchBox2optionPrice.value
  return totalPrice
})
const submitLunchbox2 = () => {
  ...
  if (totalMainQuantity !== lunchBox2TotalQuantity.value || totalDrinkQuantity !== lunchBox2TotalQuantity.value) {
    alert('主餐、飲料和總套數必須相等！')
    return
  }
  // ------------------------------------------------------------------
  ...
  for (const sideDish of lunchBox2SideDishes) {
    if (sideDish.quantity > 0) {
      row.sideDishes += sideDish.name + 'x' + parseInt(sideDish.quantity) + '<br>'
      // 保留配菜名稱、配菜 ID
      const productInput = {
...
watch(lunchBox3Mains, newMains => {
  ...
  lunchBox3Arbitrarily.value = Math.max(0, parseInt(lunchBox3TotalQuantity.value) - mainQuantityTotal)
})
...
const lunchbox3finalTotal = computed(() => {
  const totalPrice = lunchBox3TotalQuantityPrice.value + lunchBox3optionPrice.value
  return totalPrice
})
const submitLunchbox3 = () => {
  ...
  if (totalMainQuantity !== lunchBox3TotalQuantity.value || totalDrinkQuantity !== lunchBox3TotalQuantity.value) {
    alert('主餐、飲料和總套數必須相等！')
    return
  }
  // ------------------------------------------------------------------
...
watch(lunchBox4Mains, newMains => {
  ...
  lunchBox4Arbitrarily.value = Math.max(0, parseInt(lunchBox4TotalQuantity.value) - mainQuantityTotal)
})
...
const lunchbox4finalTotal = computed(() => {
  const totalPrice = lunchBox4TotalQuantityPrice.value + lunchBox4optionPrice.value
  return totalPrice
})
...
const submitLunchbox4 = () => {
  ...
  if (totalMainQuantity !== lunchBox4TotalQuantity.value || totalDrinkQuantity !== lunchBox4TotalQuantity.value) {
    alert('主餐、飲料和總套數必須相等！')
    return
  }
  // ------------------------------------------------------------------
...
const bangdongCustom1finaltotal = computed(() => {
  return bangdongCustom1TotalQuantityPrice.value + bangdongCustom1optionPrice.value
})
// 將資料推進 tableRows
const submitBangdongCustom1 = () => {
  ...
  for (const bangdongCustom1Main of bangdongCustom1Mains) {
    if (bangdongCustom1Main.quantity > 0) {
      row.Main += bangdongCustom1Main.name + 'x' + parseInt(bangdongCustom1Main.quantity).toString() + '<br>'
      const productInput = {
  ...
  for (const bangdongCustom1Drink of bangdongCustom1Drinks) {
    if (bangdongCustom1Drink.quantity > 0) {
      row.drinks += bangdongCustom1Drink.name + 'x' + parseInt(bangdongCustom1Drink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
  ...
  for (const bangdongCustom1SideDish of bangdongCustom1SideDishes) {
    if (bangdongCustom1SideDish.quantity > 0) {
      row.sideDishes += bangdongCustom1SideDish.name + 'x' + parseInt(bangdongCustom1SideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、配菜 ID
      const productInput = {
  ...
  for (const bangdongCustom1BentoMain of bangdongCustom1BentoMains) {
    if (bangdongCustom1BentoMain.quantity > 0) {
      row.bentoMains += bangdongCustom1BentoMain.name + 'x' + parseInt(bangdongCustom1BentoMain.quantity).toString() + '<br>'
      // 保留副主餐名稱、副主餐 ID
      const productInput = {
...
const lunchBoxCustom1finaltotal = computed(() => {
  return lunchBoxCustom1TotalQuantityPrice.value + lunchBoxCustom1optionPrice.value
})
const submitLunchboxCustom1 = () => {
  ...
  for (const lunchBoxCustom1BentoMain of lunchBoxCustom1BentoMains) {
    if (lunchBoxCustom1BentoMain.quantity > 0) {
      ...
      row.bentoMains += lunchBoxCustom1BentoMain.name + 'x' + parseInt(lunchBoxCustom1BentoMain.quantity).toString() + '<br>'
      // 保留副主餐名稱、副主餐 ID
      const productInput = {
  ...
  for (const lunchBoxCustom1Burrito6inch of lunchBoxCustom1Burrito6inchs) {
    if (lunchBoxCustom1Burrito6inch.quantity > 0) {
      ...
      row.bentoMains += lunchBoxCustom1Burrito6inch.name + 'x' + parseInt(lunchBoxCustom1Burrito6inch.quantity).toString() + '<br>'
      // 保留副主餐名稱、副主餐 ID
      const productInput = {
...
const submitBangdongSolo = () => {
  ...
  for (const bangdongSolo6inch of bangdongSolo6inchs) {
    if (bangdongSolo6inch.quantity > 0) {
      const bangdongSolo6inchComposition = `${bangdongSolo6inch.name} x${bangdongSolo6inch.quantity}` + '<br>'
  ...
  for (const bangdongSoloDrink of bangdongSoloDrinks) {
    if (bangdongSoloDrink.quantity > 0) {
      row.drinks += bangdongSoloDrink.name + 'x' + parseInt(bangdongSoloDrink.quantity).toString() + '<br>'
      // 保留飲料名稱、ID
      const productInput = {
  ...
  for (const bangdongSoloSideDish of bangdongSoloSideDishes) {
    if (bangdongSoloSideDish.quantity > 0) {
      const bangdongSoloSideDishComposition = `${bangdongSoloSideDish.name} x ${bangdongSoloSideDish.quantity}` + '<br>'
  ...
  for (const bangdongSoloBentoMain of bangdongSoloBentoMains) {
    if (bangdongSoloBentoMain.quantity > 0) {
      const bangdongSoloBentoMainComposition = `${bangdongSoloBentoMain.name} x${bangdongSoloBentoMain.quantity}` + '<br>'
  ...
  for (const bangdongSoloEcoBag of bangdongSoloEcoBags) {
    if (bangdongSoloEcoBag.quantity > 0) {
      const ecoBagComposition = `${bangdongSoloEcoBag.name} x${bangdongSoloEcoBag.quantity}` + '<br>'
...
function editBangdong1(row) {
  ...
  for (const input of row.inputs) {
    ...
    // 招牌便當  主餐  推入 input 數值
    const mainIndex = bangdong1EditMains.findIndex(bangdong1EditMain => bangdong1EditMain.name === name || bangdong1EditMain.shortname === name)
    if (mainIndex >= 0) {
      bangdong1EditMains[mainIndex].quantity = parseInt(value)
    }
    // 招牌便當  飲料  推入 input 數值
    const drinkIndex = bangdong1EditDrinks.findIndex(bangdong1EditDrink => bangdong1EditDrink.name === name)
    if (drinkIndex >= 0) {
      bangdong1EditDrinks[drinkIndex].quantity = parseInt(value)
    }
    // 招牌便當  配菜  推入 input 數值
    setTimeout(() => {
      const sideDishIndex = bangdong1EditSideDishes.findIndex(bangdong1EditSideDish => bangdong1EditSideDish.name === name)
      if (sideDishIndex >= 0) {
...
watch(bangdong1EditMains, newMains => {
  ...
  bangdong1EditArbitrarily.value = Math.max(0, parseInt(bangdong1EditTotalQuantity.value) - mainQuantityTotal)
})
...
const bangdong1EditTotalQuantityPrice = computed(() => {
  const totalPrice = bangdong1EditTotalQuantity.value * bangdong1EditPrice.value
  return totalPrice
})
...
const bangdong1EditfinalTotal = computed(() => {
  const totalPrice = bangdong1EditoptionPrice.value + bangdong1EditTotalQuantityPrice.value
  return totalPrice
})
// 關閉修正欄位
...
function saveEditDialog1() {
  ...
  // 檢查總數量是否匹配
  const calculatedTotalQuantity = bangdong1EditMains.reduce((acc, main) => acc + (main.quantity || 0), 0)
  console.log(calculatedTotalQuantity, bangdong1EditTotalQuantity.value)
  if (calculatedTotalQuantity !== parseInt(bangdong1EditTotalQuantity.value, 10)) {
    alert('主餐總數量與輸入的總數量不搭！')
    return
  }
  const row = {
  ...
  for (const bangdong1EditMain of bangdong1EditMains) {
    if (bangdong1EditMain.quantity > 0) {
      row.Main += bangdong1EditMain.name + 'x' + parseInt(bangdong1EditMain.quantity).toString() + '<br>'
      // 保留 主餐名稱、主餐 ID
      const productInput = {
        productTypeId: bangdong1MainMealId.value,
        productTypeName: bangdong1MainName.value,
  ...
  for (const bangdong1EditDrink of bangdong1EditDrinks) {
    if (bangdong1EditDrink.quantity > 0) {
      row.drinks += bangdong1EditDrink.name + 'x' + parseInt(bangdong1EditDrink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
        productTypeName: bangdong1DrinkName.value,
        productTypeId: bangdong1DrinkId.value,
  ...
  for (const bangdong1EditSideDish of bangdong1EditSideDishes) {
    if (bangdong1EditSideDish.quantity > 0) {
      row.sideDishes += bangdong1EditSideDish.name + 'x' + parseInt(bangdong1EditSideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、配菜 ID
      const productInput = {
        productTypeName: bangdong1SideDishName.value,
        productTypeId: bangdong1SideDishId.value,
...
function editBangdong2(row) {
  ...
  for (const input of row.inputs) {
    ...
    // 雞胸便當  主餐  推入 input 數值
    const mainIndex = bangdong2EditMains.findIndex(bangdong2EditMain => bangdong2EditMain.name === name || bangdong2EditMain.shortname === name)
    if (mainIndex >= 0) {
      bangdong2EditMains[mainIndex].quantity = parseInt(value)
    }
    // 雞胸便當  飲料  推入 input 數值
    const drinkIndex = bangdong2EditDrinks.findIndex(bangdong2EditDrink => bangdong2EditDrink.name === name)
    if (drinkIndex >= 0) {
      bangdong2EditDrinks[drinkIndex].quantity = parseInt(value)
    }
    /// / 雞胸便當  配菜  推入 input 數值
    setTimeout(() => {
      const sideDishIndex = bangdong2EditSideDishes.findIndex(bangdong2EditSideDish => bangdong2EditSideDish.name === name)
      if (sideDishIndex >= 0) {
        bangdong2EditSideDishes[sideDishIndex].quantity = parseInt(value)
      }
      // 雞胸便當  副主餐  推入 input 數值
      const bentoMainIndex = bangdong2EditBentoMains.findIndex(bangdong2EditBentoMain => bangdong2EditBentoMain.name === name)
      if (bentoMainIndex >= 0) {
...
watch(bangdong2EditMains, newMains => {
  ...
  bangdong2EditArbitrarily.value = Math.max(0, parseInt(bangdong2EditTotalQuantity.value) - mainQuantityTotal)
})
// 雞胸便當單價
...
const bangdong2EditTotalQuantityPrice = computed(() => {
  const totalPrice = bangdong2EditTotalQuantity.value * bangdong2EditPrice.value
...
const bangdong2EditfinalTotal = computed(() => {
  const totalPrice = bangdong2EditoptionPrice.value + bangdong2EditTotalQuantityPrice.value
  return totalPrice
})
// 關閉修正欄位
...
function saveEditDialog2() {
  ...
  // 檢查總數量是否匹配
  const calculatedTotalQuantity = bangdong2EditMains.reduce((acc, main) => acc + (main.quantity || 0), 0)
  console.log(calculatedTotalQuantity, bangdong2EditTotalQuantity.value)
  if (calculatedTotalQuantity !== parseInt(bangdong2EditTotalQuantity.value, 10)) {
    alert('主餐總數量與輸入的總數量不搭！')
    return
  }
  const row = {
  ...
  for (const bangdong2EditMain of bangdong2EditMains) {
    if (bangdong2EditMain.quantity > 0) {
      row.Main += bangdong2EditMain.name + 'x' + parseInt(bangdong2EditMain.quantity).toString() + '<br>'
      // 保留 主餐名稱、主餐 ID
      const productInput = {
        productTypeId: bangdong2MainMealId.value,
        productTypeName: bangdong2MainName.value,
  ...
  for (const bangdong2EditDrink of bangdong2EditDrinks) {
    if (bangdong2EditDrink.quantity > 0) {
      row.drinks += bangdong2EditDrink.name + 'x' + parseInt(bangdong2EditDrink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
        productTypeName: bangdong2DrinkName.value,
        productTypeId: bangdong2DrinkId.value,
  ...
  for (const bangdong2EditSideDish of bangdong2EditSideDishes) {
    if (bangdong2EditSideDish.quantity > 0) {
      row.sideDishes += bangdong2EditSideDish.name + 'x' + parseInt(bangdong2EditSideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、ID
      const productInput = {
        productTypeName: bangdong2SideDishName.value,
        productTypeId: bangdong2SideDishId.value,
  ...
  for (const bangdong2EditBentoMain of bangdong2EditBentoMains) {
    if (bangdong2EditBentoMain.quantity > 0) {
      row.bentoMains += bangdong2EditBentoMain.name + 'x' + parseInt(bangdong2EditBentoMain.quantity).toString() + '<br>'
      const productInput = {
        productTypeName: bangdong2BentoMainName.value,
        productTypeId: bangdong2BentoMainId.value,
        productType: 'options_with_qty',
...
function editBangdong3(row) {
  ...
  for (const input of row.inputs) {
    ...
    // 雞腿便當  主餐  推入 input 數值
    const mainIndex = bangdong3EditMains.findIndex(bangdong3EditMain => bangdong3EditMain.name === name || bangdong3EditMain.shortname === name)
    if (mainIndex >= 0) {
      bangdong3EditMains[mainIndex].quantity = parseInt(value)
    }
    // 雞腿便當  飲料  推入 input 數值
    const drinkIndex = bangdong3EditDrinks.findIndex(bangdong3EditDrink => bangdong3EditDrink.name === name)
    if (drinkIndex >= 0) {
      bangdong3EditDrinks[drinkIndex].quantity = parseInt(value)
    }
    setTimeout(() => {
      // 雞腿便當  配菜  推入 input 數值
      const sideDishIndex = bangdong3EditSideDishes.findIndex(bangdong3EditSideDish => bangdong3EditSideDish.name === name)
      if (sideDishIndex >= 0) {
        bangdong3EditSideDishes[sideDishIndex].quantity = parseInt(value)
      }
      // 雞腿便當  副主餐  推入 input 數值
      const bentoMainIndex = bangdong3EditBentoMains.findIndex(bangdong3EditBentoMain => bangdong3EditBentoMain.name === name)
      if (bentoMainIndex >= 0) {
...
watch(bangdong3EditMains, newMains => {
  ...
  bangdong3EditArbitrarily.value = Math.max(0, parseInt(bangdong3EditTotalQuantity.value) - mainQuantityTotal)
})
// 雞腿便當單價
...
const bangdong3EditTotalQuantityPrice = computed(() => {
  const totalPrice = bangdong3EditTotalQuantity.value * bangdong3EditPrice.value
  return totalPrice
})
...
const bangdong3EditfinalTotal = computed(() => {
  const totalPrice = bangdong3EditoptionPrice.value + bangdong3EditTotalQuantityPrice.value
  return totalPrice
})
// 關閉修正欄位
...
function saveEditDialog3() {
  ...
  // 檢查總數量是否匹配
  const calculatedTotalQuantity = bangdong3EditMains.reduce((acc, main) => acc + (main.quantity || 0), 0)
  console.log(calculatedTotalQuantity, bangdong3EditTotalQuantity.value)
  if (calculatedTotalQuantity !== parseInt(bangdong3EditTotalQuantity.value, 10)) {
    alert('主餐總數量與輸入的總數量不搭！')
    return
  }
  const row = {
  ...
  for (const bangdong3EditMain of bangdong3EditMains) {
    if (bangdong3EditMain.quantity > 0) {
      row.Main += bangdong3EditMain.name + ' x ' + parseInt(bangdong3EditMain.quantity).toString() + '<br>'
  ...
  for (const bangdong3EditDrink of bangdong3EditDrinks) {
    if (bangdong3EditDrink.quantity > 0) {
      row.drinks += bangdong3EditDrink.name + ' x ' + parseInt(bangdong3EditDrink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
        productTypeName: bangdong3DrinkName.value,
        productTypeId: bangdong3DrinkId.value,
  ...
  for (const bangdong3EditSideDish of bangdong3EditSideDishes) {
    if (bangdong3EditSideDish.quantity > 0) {
      row.sideDishes += bangdong3EditSideDish.name + 'x' + parseInt(bangdong3EditSideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、配菜 ID
      const productInput = {
        productTypeName: bangdong3SideDishName.value,
        productTypeId: bangdong3SideDishId.value,
  ...
  if (bangdong3EditArbitrarily.value > 0) {
    row.Main += '任意搭配' + 'x' + parseInt(bangdong3EditArbitrarily.value).toString() + '<br>'
    const input = {
...
function editBangdong4(row) {
  ...
  for (const input of row.inputs) {
    ...
    // 滷牛便當  主餐  推入 input 數值
    const mainIndex = bangdong4EditMains.findIndex(bangdong4EditMain => bangdong4EditMain.name === name || bangdong4EditMain.shortname === name)
    if (mainIndex >= 0) {
      bangdong4EditMains[mainIndex].quantity = parseInt(value)
    }
    // 滷牛便當  飲料  推入 input 數值
    const drinkIndex = bangdong4EditDrinks.findIndex(bangdong4EditDrink => bangdong4EditDrink.name === name)
    if (drinkIndex >= 0) {
      bangdong4EditDrinks[drinkIndex].quantity = parseInt(value)
    }
    setTimeout(() => {
      // 滷牛便當  配菜  推入 input 數值
      const sideDishIndex = bangdong4EditSideDishes.findIndex(bangdong4EditSideDish => bangdong4EditSideDish.name === name)
      if (sideDishIndex >= 0) {
        bangdong4EditSideDishes[sideDishIndex].quantity = parseInt(value)
      }
      // 滷牛便當  副主餐  推入 input 數值
      const bentoMainIndex = bangdong4EditBentoMains.findIndex(bangdong4EditBentoMain => bangdong4EditBentoMain.name === name)
      if (bentoMainIndex >= 0) {
...
watch(bangdong4EditMains, newMains => {
  ...
  bangdong4EditArbitrarily.value = Math.max(0, parseInt(bangdong4EditTotalQuantity.value) - mainQuantityTotal)
})
// 滷牛便當單價
...
const bangdong4EditTotalQuantityPrice = computed(() => {
  const totalPrice = bangdong4EditTotalQuantity.value * bangdong4EditPrice.value
  return totalPrice
})
...
const bangdong4EditfinalTotal = computed(() => {
  const totalPrice = bangdong4EditoptionPrice.value + bangdong4EditTotalQuantityPrice.value
  return totalPrice
})
// 關閉修正欄位
...
function saveEditDialog4() {
  ...
  // 檢查總數量是否匹配
  const calculatedTotalQuantity = bangdong4EditMains.reduce((acc, main) => acc + (main.quantity || 0), 0)
  console.log(calculatedTotalQuantity, bangdong4EditTotalQuantity.value)
  if (calculatedTotalQuantity !== parseInt(bangdong4EditTotalQuantity.value, 10)) {
    alert('主餐總數量與輸入的總數量不搭！')
    return
  }
  const row = {
  ...
  for (const bangdong4EditMain of bangdong4EditMains) {
    if (bangdong4EditMain.quantity > 0) {
      row.Main += bangdong4EditMain.name + 'x' + parseInt(bangdong4EditMain.quantity).toString() + '<br>'
      // 保留 主餐名稱、主餐 ID
      const productInput = {
        productTypeId: bangdong4MainMealId.value,
        productTypeName: bangdong4MainName.value,
  ...
  for (const bangdong4EditDrink of bangdong4EditDrinks) {
    if (bangdong4EditDrink.quantity > 0) {
      row.drinks += bangdong4EditDrink.name + 'x' + parseInt(bangdong4EditDrink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
        productTypeName: bangdong4DrinkName.value,
        productTypeId: bangdong4DrinkId.value,
  ...
  for (const bangdong4EditSideDish of bangdong4EditSideDishes) {
    if (bangdong4EditSideDish.quantity > 0) {
      row.sideDishes += bangdong4EditSideDish.name + 'x' + parseInt(bangdong4EditSideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、配菜 ID
      const productInput = {
        productTypeName: bangdong4SideDishName.value,
        productTypeId: bangdong4SideDishId.value,
  ...
  for (const bangdong4EditBentoMain of bangdong4EditBentoMains) {
    if (bangdong4EditBentoMain.quantity > 0) {
      row.bentoMains += bangdong4EditBentoMain.name + 'x' + parseInt(bangdong4EditBentoMain.quantity).toString() + '<br>'
      // 保留副主餐名稱、副主餐 ID
      const productInput = {
        productTypeName: bangdong4BentoMainName.value,
        productTypeId: bangdong4BentoMainId.value,
...
function editBangdongSalmon(row) {
  ...
  for (const input of row.inputs) {
    ...
    // 滷牛便當  主餐  推入 input 數值
    const mainIndex = bangdongSalmonEditMains.findIndex(bangdongSalmonEditMain => bangdongSalmonEditMain.name === name || bangdongSalmonEditMain.shortname === name)
    if (mainIndex >= 0) {
      bangdongSalmonEditMains[mainIndex].quantity = parseInt(value)
    }
    // 滷牛便當  飲料  推入 input 數值
    const drinkIndex = bangdongSalmonEditDrinks.findIndex(bangdongSalmonEditDrink => bangdongSalmonEditDrink.name === name)
    if (drinkIndex >= 0) {
      bangdongSalmonEditDrinks[drinkIndex].quantity = parseInt(value)
    }
    // salmon  配菜  推入 input 數值
    setTimeout(() => {
      const sideDishIndex = bangdongSalmonEditSideDishes.findIndex(bangdongSalmonEditSideDish => bangdongSalmonEditSideDish.name === name)
      if (sideDishIndex >= 0) {
        bangdongSalmonEditSideDishes[sideDishIndex].quantity = parseInt(value)
      }
      // salmon  副主餐  推入 input 數值
      const bentoMainIndex = bangdongSalmonEditBentoMains.findIndex(bangdongSalmonEditBentoMain => bangdongSalmonEditBentoMain.name === name)
      if (bentoMainIndex >= 0) {
...
watch(bangdongSalmonEditMains, newMains => {
  ...
  bangdongSalmonEditArbitrarily.value = Math.max(0, parseInt(bangdongSalmonEditTotalQuantity.value) - mainQuantityTotal)
})
...
const bangdongSalmonEditTotalQuantityPrice = computed(() => {
  const totalPrice = bangdongSalmonEditTotalQuantity.value * bangdongSalmonEditPrice.value
  return totalPrice
})
...
const bangdongSalmonEditfinalTotal = computed(() => {
  const totalPrice = bangdongSalmonEditoptionPrice.value + bangdongSalmonEditTotalQuantityPrice.value
  return totalPrice
})
// 關閉修正欄位
...
function saveEditDialogSalmon() {
  ...
  // 檢查總數量是否匹配
  const calculatedTotalQuantity = bangdongSalmonEditMains.reduce((acc, main) => acc + (main.quantity || 0), 0)
  console.log(calculatedTotalQuantity, bangdongSalmonEditTotalQuantity.value)
  if (calculatedTotalQuantity !== parseInt(bangdongSalmonEditTotalQuantity.value, 10)) {
    alert('主餐總數量與輸入的總數量不搭！')
    return
  }
  const row = {
  ...
  for (const bangdongSalmonEditMain of bangdongSalmonEditMains) {
    if (bangdongSalmonEditMain.quantity > 0) {
      row.Main += bangdongSalmonEditMain.name + 'x' + parseInt(bangdongSalmonEditMain.quantity).toString() + '<br>'
      // 保留 主餐名稱、主餐 ID
      const productInput = {
        productTypeId: bangdongSalmonMainMealId.value,
        productTypeName: bangdongSalmonMainName.value,
  ...
  for (const bangdongSalmonEditDrink of bangdongSalmonEditDrinks) {
    if (bangdongSalmonEditDrink.quantity > 0) {
      row.drinks += bangdongSalmonEditDrink.name + 'x' + parseInt(bangdongSalmonEditDrink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
        productTypeName: bangdongSalmonDrinkName.value,
        productTypeId: bangdongSalmonDrinkId.value,
  ...
  for (const bangdongSalmonEditSideDish of bangdongSalmonEditSideDishes) {
    if (bangdongSalmonEditSideDish.quantity > 0) {
      row.sideDishes += bangdongSalmonEditSideDish.name + 'x' + parseInt(bangdongSalmonEditSideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、配菜 ID
      const productInput = {
        productTypeName: bangdongSalmonSideDishName.value,
        productTypeId: bangdongSalmonSideDishId.value,
  ...
  for (const bangdongSalmonEditBentoMain of bangdongSalmonEditBentoMains) {
    if (bangdongSalmonEditBentoMain.quantity > 0) {
      row.bentoMains += bangdongSalmonEditBentoMain.name + 'x' + parseInt(bangdongSalmonEditBentoMain.quantity).toString() + '<br>'
      // 保留副主餐名稱、副主餐 ID
      const productInput = {
        productTypeName: bangdongSalmonBentoMainName.value,
        productTypeId: bangdongSalmonBentoMainId.value,
  ...
  if (bangdongSalmonEditArbitrarily.value > 0) {
    row.Main += '任意搭配' + 'x' + parseInt(bangdongSalmonEditArbitrarily.value).toString()
    const input = {
...
function editBangdongVegetarian1(row) {
  ...
  for (const input of row.inputs) {
    ...
    const mainIndex = bangdongVegetarian1EditMains.findIndex(
      bangdongVegetarian1EditMain => bangdongVegetarian1EditMain.name === name || bangdongVegetarian1EditMain.shortname === name
    )
    if (mainIndex >= 0) {
      bangdongVegetarian1EditMains[mainIndex].quantity = parseInt(value)
    }
    // 素食便當 125  飲料  推入 input 數值
    const drinkIndex = bangdongVegetarian1EditDrinks.findIndex(bangdongVegetarian1EditDrink => bangdongVegetarian1EditDrink.name === name)
    if (drinkIndex >= 0) {
      bangdongVegetarian1EditDrinks[drinkIndex].quantity = parseInt(value)
    }
    setTimeout(() => {
      // 素食便當 125  配菜  推入 input 數值
      const sideDishIndex = bangdongVegetarian1EditSideDishes.findIndex(bangdongVegetarian1EditSideDish => bangdongVegetarian1EditSideDish.name === name)
      if (sideDishIndex >= 0) {
        bangdongVegetarian1EditSideDishes[sideDishIndex].quantity = parseInt(value)
      }
      // 素食便當 125  副主餐  推入 input 數值
      const bentoMainIndex = bangdongVegetarian1EditBentoMains.findIndex(bangdongVegetarian1EditBentoMain => bangdongVegetarian1EditBentoMain.name === name)
      if (bentoMainIndex >= 0) {
...
watch(bangdongVegetarian1EditMains, newMains => {
  ...
  bangdongVegetarian1EditArbitrarily.value = Math.max(0, parseInt(bangdongVegetarian1EditTotalQuantity.value) - mainQuantityTotal)
})
// 素食便當 125 單價
...
const bangdongVegetarian1EditTotalQuantityPrice = computed(() => {
  const totalPrice = bangdongVegetarian1EditTotalQuantity.value * bangdongVegetarian1EditPrice.value
  return totalPrice
})
...
const bangdongVegetarian1EditfinalTotal = computed(() => {
  const totalPrice = bangdongVegetarian1EditoptionPrice.value + bangdongVegetarian1EditTotalQuantityPrice.value
  return totalPrice
})
// 關閉修正欄位
...
function saveEditDialogV1() {
  ...
  // 檢查總數量是否匹配
  const calculatedTotalQuantity = bangdongVegetarian1EditMains.reduce((acc, main) => acc + (main.quantity || 0), 0)
  console.log(calculatedTotalQuantity, bangdongVegetarian1EditTotalQuantity.value)
  if (calculatedTotalQuantity !== parseInt(bangdongVegetarian1EditTotalQuantity.value, 10)) {
    alert('主餐總數量與輸入的總數量不搭！')
    return
  }
  const row = {
  ...
  for (const bangdongVegetarian1EditMain of bangdongVegetarian1EditMains) {
    if (bangdongVegetarian1EditMain.quantity > 0) {
      row.Main += bangdongVegetarian1EditMain.name + 'x' + parseInt(bangdongVegetarian1EditMain.quantity).toString() + '<br>'
      // 保留 主餐名稱、主餐 ID
      const productInput = {
        productTypeId: bangdongVegetarian1MainMealId.value,
        productTypeName: bangdongVegetarian1MainName.value,
  ...
  for (const bangdongVegetarian1EditDrink of bangdongVegetarian1EditDrinks) {
    if (bangdongVegetarian1EditDrink.quantity > 0) {
      row.drinks += bangdongVegetarian1EditDrink.name + 'x' + parseInt(bangdongVegetarian1EditDrink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
        productTypeName: bangdongVegetarian1DrinkName.value,
        productTypeId: bangdongVegetarian1DrinkId.value,
  ...
  for (const bangdongVegetarian1EditSideDish of bangdongVegetarian1EditSideDishes) {
    if (bangdongVegetarian1EditSideDish.quantity > 0) {
      row.sideDishes += bangdongVegetarian1EditSideDish.name + 'x' + parseInt(bangdongVegetarian1EditSideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、ID
      const productInput = {
        productTypeName: bangdongVegetarian1SideDishName.value,
        productTypeId: bangdongVegetarian1SideDishId.value,
  ...
  for (const bangdongVegetarian1EditBentoMain of bangdongVegetarian1EditBentoMains) {
    if (bangdongVegetarian1EditBentoMain.quantity > 0) {
      row.bentoMains += bangdongVegetarian1EditBentoMain.name + 'x' + parseInt(bangdongVegetarian1EditBentoMain.quantity).toString() + '<br>'
      const productInput = {
        productTypeName: bangdongVegetarian1BentoMainName.value,
        productTypeId: bangdongVegetarian1BentoMainId.value,
        productType: 'options_with_qty',
  ...
  if (bangdongVegetarian1EditArbitrarily.value > 0) {
    row.Main += '任意搭配' + 'x' + parseInt(bangdongVegetarian1EditArbitrarily.value).toString() + '<br>'
    const input = {
...
function editBangdongVegetarian2(row) {
  ...
  for (const input of row.inputs) {
    ...
    const mainIndex = bangdongVegetarian2EditMains.findIndex(
      bangdongVegetarian2EditMain => bangdongVegetarian2EditMain.name === name || bangdongVegetarian2EditMain.shortname === name
    )
    if (mainIndex >= 0) {
      bangdongVegetarian2EditMains[mainIndex].quantity = parseInt(value)
    }
    // 素食便當 150 飲料 推入 input 數值
    const drinkIndex = bangdongVegetarian2EditDrinks.findIndex(bangdongVegetarian2EditDrink => bangdongVegetarian2EditDrink.name === name)
    if (drinkIndex >= 0) {
      bangdongVegetarian2EditDrinks[drinkIndex].quantity = parseInt(value)
    }
    setTimeout(() => {
      // 素食便當 150 配菜 推入 input 數值
      const sideDishIndex = bangdongVegetarian2EditSideDishes.findIndex(bangdongVegetarian2EditSideDish => bangdongVegetarian2EditSideDish.name === name)
      if (sideDishIndex >= 0) {
        bangdongVegetarian2EditSideDishes[sideDishIndex].quantity = parseInt(value)
      }
      // 素食便當 150 副主餐 推入 input 數值
      const bentoMainIndex = bangdongVegetarian2EditBentoMains.findIndex(bangdongVegetarian2EditBentoMain => bangdongVegetarian2EditBentoMain.name === name)
      if (bentoMainIndex >= 0) {
...
watch(bangdongVegetarian2EditMains, newMains => {
  ...
  bangdongVegetarian2EditArbitrarily.value = Math.max(0, parseInt(bangdongVegetarian2EditTotalQuantity.value) - mainQuantityTotal)
})
// 素食便當 150 單價
...
const bangdongVegetarian2EditTotalQuantityPrice = computed(() => {
  const totalPrice = bangdongVegetarian2EditTotalQuantity.value * bangdongVegetarian2EditPrice.value
  return totalPrice
})
...
const bangdongVegetarian2EditfinalTotal = computed(() => {
  const totalPrice = bangdongVegetarian2EditoptionPrice.value + bangdongVegetarian2EditTotalQuantityPrice.value
  return totalPrice
})
// 關閉修正欄位
...
function saveEditDialogV2() {
  ...
  // 檢查總數量是否匹配
  const calculatedTotalQuantity = bangdongVegetarian2EditMains.reduce((acc, main) => acc + (main.quantity || 0), 0)
  if (calculatedTotalQuantity !== parseInt(bangdongVegetarian2EditTotalQuantity.value, 10)) {
    alert('主餐總數量與輸入的總數量不搭！')
    return
  }
  const row = {
  ...
  for (const bangdongVegetarian2EditMain of bangdongVegetarian2EditMains) {
    if (bangdongVegetarian2EditMain.quantity > 0) {
      row.Main += bangdongVegetarian2EditMain.name + 'x' + parseInt(bangdongVegetarian2EditMain.quantity).toString() + '<br>'
      // 保留 主餐名稱、主餐 ID
      const productInput = {
        productTypeId: bangdongVegetarian2MainMealId.value,
        productTypeName: bangdongVegetarian2MainName.value,
  ...
  for (const bangdongVegetarian2EditDrink of bangdongVegetarian2EditDrinks) {
    if (bangdongVegetarian2EditDrink.quantity > 0) {
      row.drinks += bangdongVegetarian2EditDrink.name + 'x' + parseInt(bangdongVegetarian2EditDrink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
        productTypeName: bangdongVegetarian2DrinkName.value,
        productTypeId: bangdongVegetarian2DrinkId.value,
  ...
  for (const bangdongVegetarian2EditSideDish of bangdongVegetarian2EditSideDishes) {
    if (bangdongVegetarian2EditSideDish.quantity > 0) {
      row.sideDishes += bangdongVegetarian2EditSideDish.name + 'x' + parseInt(bangdongVegetarian2EditSideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、ID
      const productInput = {
        productTypeName: bangdongVegetarian2SideDishName.value,
        productTypeId: bangdongVegetarian2SideDishId.value,
  ...
  for (const bangdongVegetarian2EditBentoMain of bangdongVegetarian2EditBentoMains) {
    if (bangdongVegetarian2EditBentoMain.quantity > 0) {
      row.bentoMains += bangdongVegetarian2EditBentoMain.name + 'x' + parseInt(bangdongVegetarian2EditBentoMain.quantity).toString() + '<br>'
      const productInput = {
        productTypeName: bangdongVegetarian2BentoMainName.value,
        productTypeId: bangdongVegetarian2BentoMainId.value,
        productType: 'options_with_qty',
  ...
  if (bangdongVegetarian2EditArbitrarily.value > 0) {
    row.Main += '任意搭配' + 'x' + parseInt(bangdongVegetarian2EditArbitrarily.value).toString() + '<br>'
    const input = {
...
const bangdongCustom1EditfinalTotal = computed(() => {
  const totalPrice = bangdongCustom1EditoptionPrice.value + bangdongCustom1EditTotalQuantityPrice.value
  return totalPrice
})
...
function editRowDialog9(row) {
  ...
  for (const input of row.inputs) {
    ...
    const mainIndex = bangdongCustom1EditMains.findIndex(bangdongCustom1EditMain => bangdongCustom1EditMain.name === name || bangdongCustom1EditMain.short_name === name)
    if (mainIndex >= 0) {
      bangdongCustom1EditMains[mainIndex].quantity = parseInt(value)
    }
    // 客製招牌便當  飲料  推入 input 數值
    const drinkIndex = bangdongCustom1EditDrinks.findIndex(bangdongCustom1EditDrink => bangdongCustom1EditDrink.name === name)
    if (drinkIndex >= 0) {
      bangdongCustom1EditDrinks[drinkIndex].quantity = parseInt(value)
    }
    // 客製招牌便當  配菜  推入 input 數值
    const sideDishIndex = bangdongCustom1EditSideDishes.findIndex(bangdongCustom1EditSideDish => bangdongCustom1EditSideDish.name === name)
    if (sideDishIndex >= 0) {
      bangdongCustom1EditSideDishes[sideDishIndex].quantity = parseInt(value)
    }
    // 客製招牌便當  副主餐  推入 input 數值
    const bentoMainIndex = bangdongCustom1EditBentoMains.findIndex(bangdongCustom1EditBentoMain => bangdongCustom1EditBentoMain.name === name)
    if (bentoMainIndex >= 0) {
...
function saveEditDialog9() {
  ...
  for (const bangdongCustom1EditMain of bangdongCustom1EditMains) {
    if (bangdongCustom1EditMain.quantity > 0) {
      row.Main += bangdongCustom1EditMain.name + 'x' + parseInt(bangdongCustom1EditMain.quantity).toString() + '<br>'
      const productInput = {
        productTypeId: bangdongCustom1MainMealId.value,
        productTypeName: bangdongCustom1MainName.value,
        productType: 'options_with_qty',
  ...
  for (const bangdongCustom1EditDrink of bangdongCustom1EditDrinks) {
    if (bangdongCustom1EditDrink.quantity > 0) {
      row.drinks += bangdongCustom1EditDrink.name + 'x' + parseInt(bangdongCustom1EditDrink.quantity).toString() + '<br>'
      // 保留飲料名稱、飲料 ID
      const productInput = {
        productTypeName: bangdongCustom1DrinkName.value,
        productTypeId: bangdongCustom1DrinkId.value,
  ...
  for (const bangdongCustom1EditSideDish of bangdongCustom1EditSideDishes) {
    if (bangdongCustom1EditSideDish.quantity > 0) {
      row.sideDishes += bangdongCustom1EditSideDish.name + 'x' + parseInt(bangdongCustom1EditSideDish.quantity).toString() + '<br>'
      // 保留配菜名稱、配菜 ID
      const productInput = {
        productTypeName: bangdongCustom1SideDishName.value,
        productTypeId: bangdongCustom1SideDishId.value,
  ...
  for (const bangdongCustom1EditBentoMain of bangdongCustom1EditBentoMains) {
    if (bangdongCustom1EditBentoMain.quantity > 0) {
      row.bentoMains += bangdongCustom1EditBentoMain.name + 'x' + parseInt(bangdongCustom1EditBentoMain.quantity).toString() + '<br>'
      // 保留副主餐名稱、副主餐 ID
      const productInput = {
        productTypeName: bangdongCustom1BentoMainName.value,
        productTypeId: bangdongCustom1BentoMainId.value,
...
function editRowDialog5(row) {
  ...
  for (const input of row.inputs) {
    ...
    const mainIndex = lunchBox1EditMains.findIndex(lunchBox1EditMain => lunchBox1EditMain.name === name || lunchBox1EditMain.shortname === name)
    if (mainIndex >= 0) {
...
watch(lunchBox1EditMains, newMains => {
  ...
  lunchBox1EditArbitrarily.value = Math.max(0, parseInt(lunchBox1EditTotalQuantity.value) - mainQuantityTotal)
})
...
const lunchbox1EditfinalTotal = computed(() => {
  const totalPrice = lunchBox1EditTotalQuantityPrice.value + lunchBox1EditoptionPrice.value
  return totalPrice
})
...
function editRowDialog6(row) {
  ...
  for (const input of row.inputs) {
    ...
    const mainIndex = lunchBox2EditMains.findIndex(lunchBox2EditMain => lunchBox2EditMain.name === name || lunchBox2EditMain.shortname === name)
    if (mainIndex >= 0) {
...
watch(lunchBox2EditMains, newMains => {
  ...
  lunchBox2EditArbitrarily.value = Math.max(0, parseInt(lunchBox2EditTotalQuantity.value) - mainQuantityTotal)
})
...
const lunchbox2EditfinalTotal = computed(() => {
  const totalPrice = lunchBox2EditTotalQuantityPrice.value + lunchBox2EditoptionPrice.value
  return totalPrice
})
...
const saveEditDialog6 = () => {
  ...
  if (totalMainQuantity !== parseInt(lunchBox2EditTotalQuantity.value) || totalDrinkQuantity !== parseInt(lunchBox2EditTotalQuantity.value)) {
    alert('主餐、飲料和總套數必須相等！')
    return
  }
  const row = {
...
function editRowDialog7(row) {
  ...
  for (const input of row.inputs) {
    ...
    const mainIndex = lunchBox3EditMains.findIndex(lunchBox3EditMain => lunchBox3EditMain.name === name || lunchBox3EditMain.shortname === name)
    if (mainIndex >= 0) {
...
watch(lunchBox3EditMains, newMains => {
  ...
  lunchBox3EditArbitrarily.value = Math.max(0, parseInt(lunchBox3EditTotalQuantity.value) - mainQuantityTotal)
})
...
const lunchbox3EditfinalTotal = computed(() => {
  const totalPrice = lunchBox3EditTotalQuantityPrice.value + lunchBox3EditoptionPrice.value
  return totalPrice
})
...
const saveEditDialog7 = () => {
  ...
  if (totalMainQuantity !== parseInt(lunchBox3EditTotalQuantity.value) || totalDrinkQuantity !== parseInt(lunchBox3EditTotalQuantity.value)) {
    alert('主餐、飲料和總套數必須相等！')
    return
  }
  const row = {
...
function editRowDialog8(row) {
  ...
  for (const input of row.inputs) {
    ...
    const mainIndex = lunchBox4EditMains.findIndex(lunchBox4EditMain => lunchBox4EditMain.name === name || lunchBox4EditMain.shortname === name)
    if (mainIndex >= 0) {
...
watch(lunchBox4EditMains, newMains => {
  ...
  lunchBox4EditArbitrarily.value = Math.max(0, parseInt(lunchBox4EditTotalQuantity.value) - mainQuantityTotal)
})
...
const lunchbox4EditfinalTotal = computed(() => {
  const totalPrice = lunchBox4EditTotalQuantityPrice.value + lunchBox4EditoptionPrice.value
  return totalPrice
})
...
const saveEditDialog8 = () => {
  ...
  if (totalMainQuantity !== parseInt(lunchBox4EditTotalQuantity.value) || totalDrinkQuantity !== parseInt(lunchBox4EditTotalQuantity.value)) {
    alert('主餐、飲料和總套數必須相等！')
    return
  }
  const row = {
...
function editRowDialog0(row) {
  ...
  for (const input of row.inputs) {
    ...
    const mainIndex = lunchBoxCustom1EditMains.findIndex(lunchBoxCustom1EditMain => lunchBoxCustom1EditMain.name === name || lunchBoxCustom1EditMain.short_name === name)
    if (mainIndex >= 0) {
    ...
    const bentoMainIndex = lunchBoxCustom1EditBentoMains.findIndex(lunchBoxCustom1EditBentoMain => lunchBoxCustom1EditBentoMain.name === name)
    if (bentoMainIndex >= 0) {
...
const lunchBoxCustom1Editfinaltotal = computed(() => {
  return lunchBoxCustom1EditTotalQuantityPrice.value + lunchboxCustom1EditoptionPrice.value
})
...
const saveEditDialog0 = () => {
  ...
  for (const lunchBoxCustom1EditBentoMain of lunchBoxCustom1EditBentoMains) {
    if (lunchBoxCustom1EditBentoMain.quantity > 0) {
      ...
      row.bentoMains += lunchBoxCustom1EditBentoMain.name + 'x' + parseInt(lunchBoxCustom1EditBentoMain.quantity).toString() + '<br>'
      // 保留副主餐名稱、副主餐 ID
      const productInput = {
        productTypeName: lunchBoxCustom1BentoMainName.value,
        productTypeId: lunchBoxCustom1BentoMainId.value,
...
function saveEditDialog10() {
  ...
  for (const bangdongSoloEditDrink of bangdongSoloEditDrinks) {
    if (bangdongSoloEditDrink.quantity > 0) {
      ...
      row.drinks += bangdongSoloEditDrink.name + 'x' + parseInt(bangdongSoloEditDrink.quantity).toString() + '<br>'
      // 保留飲料名稱、ID
      const productInput = {
        productTypeName: bangdongSoloDrinkName.value,
        productTypeId: bangdongSoloDrinkId.value,
...
const addOrder = async () => {
  ...
  const findAddress = async () => {
    let state = address.state.find(address => address.label === personForm.address)
    if (state === undefined) {
      state = address.state.find(address => address.value === personForm.address)
    }
    if (state !== undefined) {
      personForm.address = state.value
    }
  ...
  const findCity = async val => {
    ...
    if (personForm.address2) {
      ...
      let city = address.city.find(address => address.label === personForm.address2)
      // 如果未找到，則使用 value 進行第二次尋找
      if (city === undefined) {
        city = address.city.find(address => address.value === personForm.address2)
      }
      // 檢查是否找到匹配的項目
      if (city !== undefined) {
    ...
    for (const row of tableRows) {
      for (const productInput of row.productInputs) {
        if (Object.prototype.hasOwnProperty.call(productInput, 'name')) {
          formData.append(`order_products[${row.id}][main_category_code]`, meal.getCategoryCode(productInput.name))
          ...
          formData.append(`order_products[${row.id}][final_total]`, productInput.finaltotalPrice)
          formData.append(`order_products[${row.id}][options_total]`, productInput.optionPrice)
          formData.append(`order_products[${row.id}][comment]`, productInput.comment)
        }
      }

      let productOptionIndex = 1
      for (const productInput of row.productInputs) {
        if (typeof productInput.productTypeId !== 'undefined' && typeof productInput.productType !== 'undefined' && typeof productInput.productTypeName !== 'undefined') {
          ...
          for (const input of row.inputs) {
            if (input.mainId === productInput.mainId && input.type === productInput.type) {
...
const editOrder = async () => {
  ...
  try {
    ...
    if (isNaN(meal.order.shipping_city_id)) {
      let city = address.city.find(c => c.label === meal.order.shipping_city_id)
      if (city === undefined) {
        city = address.city.find(c => c.value === meal.order.shipping_city_id)
      }
...
const roads = computed(() => {
  ...
  return road.roads.filter(road => road.value.includes(searchTerm.value)).slice(start, end)
})
const totalPages = computed(() => Math.ceil(road.roads.length / ITEMS_PER_PAGE))
